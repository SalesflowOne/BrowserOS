...
class ExtractBaseDefaultTest(unittest.TestCase):
    ...
    def test_extract_single_commit_uses_base_commit_by_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch(
                    "bos_build.modules.extract.extract_commit.validate_commit_exists",
                    return_value=True,
                ),
                patch(
                    "bos_build.modules.extract.extract_commit.get_commit_info",
                    return_value=None,
                ),
                patch(
                    "bos_build.modules.extract.extract_commit.extract_with_base",
    ...
    def test_extract_single_file_patch_uses_base_commit_by_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch(
                    "bos_build.modules.extract.extract_patch.validate_commit_exists",
                    return_value=True,
                ) as validate_mock,
                patch(
                    "bos_build.modules.extract.extract_patch.run_git_command",
                    return_value=diff_result,
                ) as git_mock,
                patch(
                    "bos_build.modules.extract.extract_patch.parse_diff_output",
                    return_value={"chrome/foo.cc": patch_file},
                ),
                patch(
                    "bos_build.modules.extract.extract_patch.write_patch_file",
                    return_value=True,
                ),
            ):
    ...
    def test_extract_commits_individually_uses_base_commit_by_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch(
                    "bos_build.modules.extract.extract_range.validate_commit_exists",
                    return_value=True,
                ),
                patch(
                    "bos_build.modules.extract.extract_range.run_git_command",
                    return_value=rev_list,
                ),
                patch(
                    "bos_build.modules.extract.extract_range.extract_with_base",
                    return_value=(1, ["chrome/foo.cc"]),
                ) as extract_with_base_mock,
                patch(
                    "click.progressbar",

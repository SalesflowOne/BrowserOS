...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'CALENDLY_API_KEY environment variable required' }),
  )
  process.exit(1)
}
    ...
    case 'event-types':
      switch (sub) {
        case 'list': {
          ...
          if (!user && !org) {
            result = { error: '--user or --organization URI required' }
            break
          }
    ...
    case 'events':
      switch (sub) {
        ...
        case 'invitees': {
          const uuid = args.uuid
          if (!uuid) {
            result = { error: '--uuid required (event UUID)' }
            break
          }
          const params = new URLSearchParams()
          params.set('count', String(count))
          ...
          result = await api(
            'GET',
            `/scheduled_events/${uuid}/invitees?${params}`,
          )
          break
        }
        default:
          result = {
            error:
              'Unknown events subcommand. Use: list, get, cancel, invitees',
          }
      }
      break
    ...
    case 'availability':
      switch (sub) {
        case 'times': {
          ...
          if (!eventType) {
            result = { error: '--event-type URI required' }
            break
          }
          ...
          if (!startTime || !endTime) {
            result = {
              error: '--start-time and --end-time required (ISO 8601)',
            }
            break
          }
          const params = new URLSearchParams({
        ...
        case 'busy': {
          const user = args.user
          if (!user) {
            result = { error: '--user URI required' }
            break
          }
          const startTime = args['start-time']
          const endTime = args['end-time']
        ...
        default:
          result = {
            error: 'Unknown availability subcommand. Use: times, busy',
          }
      }
      break
    ...
    case 'webhooks':
      switch (sub) {
        case 'list': {
          ...
          if (!org) {
            result = { error: '--organization URI required' }
            break
          }
          const params = new URLSearchParams({
            organization: org,
        ...
        case 'create': {
          ...
          if (!url || !events || !org) {
            result = {
              error:
                '--url, --events (comma-separated), and --organization required',
            }
            break
          }
    ...
    default:
      result = {
        ...
        usage: {
          ...
          events:
            'events [list --user <uri> | get --uuid <id> | cancel --uuid <id> | invitees --uuid <id>]',
          availability:
            'availability [times --event-type <uri> --start-time <iso> --end-time <iso> | busy --user <uri> --start-time <iso> --end-time <iso>]',
          webhooks:
            'webhooks [list --organization <uri> | create --url <url> --events <e1,e2> --organization <uri> | delete --uuid <id>]',
          org: 'org [members --organization <uri>]',
          options:
            '--count <n> --page-token <token> --status <active|canceled>',
        },
      }
  }

...
if (!ACCESS_TOKEN) {
  console.error(
    JSON.stringify({ error: 'GA4_ACCESS_TOKEN environment variable required' }),
  )
  process.exit(1)
}
...
async function mpApi(measurementId, apiSecret, body) {
  ...
  if (args['dry-run']) {
    return {
      _dry_run: true,
      method: 'POST',
      url: `${MP_URL}?${new URLSearchParams({ measurement_id: measurementId, api_secret: '***' })}`,
...
async function main() {
  ...
  switch (cmd) {
    case 'reports':
      switch (sub) {
        case 'run': {
          ...
          if (!property) {
            result = { error: '--property required' }
            break
          }
          const body = {
            dateRanges: [
      ...
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            ADMIN_API,
            `/properties/${property}/conversionEvents`,
          )
          break
        }
        case 'create': {
          ...
          if (!args['event-name']) {
            result = { error: '--event-name required' }
            break
          }
          result = await api(
            'POST',
        ...
        default:
          result = {
            error: 'Unknown conversions subcommand. Use: list, create',
          }
      }
      break
    ...
    case 'events':
      switch (sub) {
        case 'send': {
          if (!args['measurement-id']) {
            result = { error: '--measurement-id required' }
            break
          }
          if (!args['api-secret']) {
            result = { error: '--api-secret required' }
            break
          }
          if (!args['client-id']) {
    ...
    default:
      result = {
        ...
        usage: {
          reports:
            'reports run --property <id> [--start-date <date>] [--end-date <date>] [--dimensions <dims>] [--metrics <metrics>]',
          realtime:
            'realtime run --property <id> [--dimensions <dims>] [--metrics <metrics>]',
          conversions:
            'conversions [list|create] --property <id> [--event-name <name>]',
          events:
            'events send --measurement-id <id> --api-secret <secret> --client-id <id> --event-name <name> [--params <json>]',
        },
      }
  }

...
if (!API_TOKEN) {
  console.error(
    JSON.stringify({
      error: 'LIVESTORM_API_TOKEN environment variable required',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    ...
    case 'events':
      switch (sub) {
        case 'list': {
          ...
          if (args.title)
            qs += `&filter[title]=${encodeURIComponent(args.title)}`
          ...
          const payload = {
            data: {
              ...
              attributes: {
                title,
                slug:
                  args.slug || title.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
              },
            },
          }
          if (args.description)
            payload.data.attributes.description = args.description
          if (args['estimated-duration'])
            payload.data.attributes.estimated_duration = Number(
              args['estimated-duration'],
            )
        ...
        case 'people': {
          ...
          const qs = `?page[number]=${page}&page[size]=${perPage}`
        ...
        default:
          result = {
            error:
              'Unknown events subcommand. Use: list, get, create, update, delete, people',
          }
      }
      break
    ...
    case 'sessions':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!eventId) {
            result = { error: '--event-id required' }
            break
          }
          const payload = {
            data: {
          ...
          if (args['estimated-started-at'])
            payload.data.attributes.estimated_started_at =
              args['estimated-started-at']
        ...
        case 'register': {
          const id = args.id
          if (!id) {
            result = { error: '--id (session id) required' }
            break
          }
        ...
        case 'unregister': {
          ...
          result = await api(
            'DELETE',
            `/sessions/${id}/people?filter[email]=${encodeURIComponent(email)}`,
          )
          break
        }
        case 'chat': {
        ...
        default:
          result = {
            error:
              'Unknown sessions subcommand. Use: list, get, create, delete, people, register, unregister, chat, questions, recordings',
          }
      }
      break
    ...
    case 'people':
      switch (sub) {
        case 'list': {
          let qs = `?page[number]=${page}&page[size]=${perPage}`
          if (args.email)
            qs += `&filter[email]=${encodeURIComponent(args.email)}`
    ...
    case 'webhooks':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown webhooks subcommand. Use: list, create, delete',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          ping: 'ping',
          events:
            'events [list | get --id <id> | create --title <t> | update --id <id> --title <t> | delete --id <id> | people --id <id>]',
          sessions:
            'sessions [list | get --id <id> | create --event-id <id> | delete --id <id> | people --id <id> | register --id <id> --email <e> | unregister --id <id> --email <e> | chat --id <id> | questions --id <id> | recordings --id <id>]',
          people: 'people [list --email <e> | get --id <id>]',
          webhooks:
            'webhooks [list | create --url <url> --event <name> | delete --id <id>]',

...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'HUNTER_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    ...
    case 'email':
      switch (sub) {
        case 'find': {
          ...
          if (!firstName) {
            result = { error: '--first-name required' }
            break
          }
          if (!lastName) {
            result = { error: '--last-name required' }
            break
          }
    ...
    case 'leads':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown leads subcommand. Use: list, get, create, delete',
          }
      }
      break
    ...
    case 'campaigns':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown campaigns subcommand. Use: list, get, start, pause',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          domain: {
            search:
              'domain search --domain <domain> [--limit <n>] [--type personal|generic]',
          ...
          leads: {
            ...
            get: 'leads get --id <id>',
            create:
              'leads create --email <email> [--first-name <name>] [--last-name <name>] [--company <company>]',

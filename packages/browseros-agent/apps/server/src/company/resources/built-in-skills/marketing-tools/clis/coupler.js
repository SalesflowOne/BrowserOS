...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'COUPLER_API_KEY environment variable required' }),
  )
  process.exit(1)
}
    ...
    case 'importers':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown importers subcommand. Use: list, get, run, create, delete',
          }
      }
      break
    ...
    case 'runs':
      switch (sub) {
        case 'list': {
          ...
          if (!importerId) {
            result = { error: '--importer-id required' }
            break
          }
          const params = new URLSearchParams()
          if (args.limit) params.set('limit', args.limit)
          ...
          result = await api(
            'GET',
            `/importers/${importerId}/runs${qs ? '?' + qs : ''}`,
          )
          break
        }
        case 'get': {
      ...
      result = {
        ...
        usage: {
          importers: {
            ...
            create:
              'importers create --source <source-type> --destination <dest-type> --name <name> [--schedule <schedule>]',

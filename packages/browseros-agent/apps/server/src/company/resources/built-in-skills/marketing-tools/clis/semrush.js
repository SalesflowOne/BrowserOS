...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'SEMRUSH_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'domain':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown domain subcommand. Use: overview, organic, competitors',
          }
      }
      break
    ...
    case 'keywords':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown keywords subcommand. Use: overview, related, difficulty',
          }
      }
      break
    ...
    case 'backlinks':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown backlinks subcommand. Use: overview, list',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          ...
          'domain organic':
            'domain organic --domain <domain> [--database <db>] [--limit <n>]',
          'domain competitors':
            'domain competitors --domain <domain> [--database <db>] [--limit <n>]',
          'keywords overview':
            'keywords overview --phrase <phrase> [--database <db>]',
          'keywords related':
            'keywords related --phrase <phrase> [--database <db>] [--limit <n>]',
          'keywords difficulty':
            'keywords difficulty --phrase <phrase> [--database <db>]',

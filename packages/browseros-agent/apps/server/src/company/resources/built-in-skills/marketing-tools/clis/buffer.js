...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'BUFFER_API_KEY environment variable required' }),
  )
  process.exit(1)
}
    ...
    case 'profiles':
      switch (sub) {
        ...
        case 'schedules': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (profile ID)' }
            break
          }
        ...
        default:
          result = {
            error: 'Unknown profiles subcommand. Use: list, get, schedules',
          }
      }
      break
    ...
    case 'updates':
      switch (sub) {
        case 'get': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (update ID)' }
            break
          }
        ...
        case 'create': {
          ...
          if (!profileIds) {
            result = { error: '--profile-ids required (comma-separated)' }
            break
          }
          if (!text) {
          ...
          profileIds
            .split(',')
            .forEach((id) => formBody.append('profile_ids[]', id.trim()))
          if (args['scheduled-at'])
            formBody.append('scheduled_at', args['scheduled-at'])
          ...
          if (args['dry-run']) {
            result = {
              _dry_run: true,
              method: 'POST',
              url: `${BASE_URL}/updates/create.json`,
              headers: {
                Authorization: '***',
          ...
          try {
            ...
            result = { status: res.status, body: resText }
          }
          break
        }
        case 'update': {
        ...
        case 'reorder': {
          ...
          if (!order) {
            result = { error: '--order required (comma-separated update IDs)' }
            break
          }
          const formBody = new URLSearchParams()
          order
            .split(',')
            .forEach((uid) => formBody.append('order[]', uid.trim()))
          if (args['dry-run']) {
            result = {
              _dry_run: true,
              method: 'POST',
              url: `${BASE_URL}/profiles/${id}/updates/reorder.json`,
              headers: {
                Authorization: '***',
                'Content-Type': 'application/x-www-form-urlencoded',
                Accept: 'application/json',
          ...
          const res = await fetch(
            `${BASE_URL}/profiles/${id}/updates/reorder.json`,
            {
              method: 'POST',
              headers: {
                Authorization: `Bearer ${API_KEY}`,
        ...
        default:
          result = {
            error:
              'Unknown updates subcommand. Use: get, pending, sent, create, update, share, destroy, reorder, shuffle',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          ...
          updates:
            'updates [get --id <id> | pending --id <profile-id> | sent --id <profile-id> | create --profile-ids <ids> --text <text> [--scheduled-at <time>] [--now] | update --id <id> --text <text> | share --id <id> | destroy --id <id> | reorder --id <profile-id> --order <id1,id2> | shuffle --id <profile-id>]',
          info: 'info',
        },
      }
  }

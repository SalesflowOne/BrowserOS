...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'INTERCOM_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'contacts':
      switch (sub) {
        case 'list': {
          ...
          if (args['starting-after'])
            params.set('starting_after', args['starting-after'])
        ...
        case 'search': {
          ...
          if (!field || !value) {
            result = { error: '--field and --value required' }
            break
          }
          const body = {
        ...
        case 'tag': {
          ...
          if (!id || !tagId) {
            result = { error: '--id (contact ID) and --tag-id required' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown contacts subcommand. Use: list, get, create, update, search, delete, tag, untag',
          }
      }
      break
    ...
    case 'conversations':
      switch (sub) {
        ...
        case 'reply': {
          ...
          if (!id || !body || !adminId) {
            result = { error: '--id, --body, and --admin-id required' }
            break
          }
        ...
        case 'close': {
          ...
          const adminId = args['admin-id']
          if (!id || !adminId) {
            result = { error: '--id and --admin-id required' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown conversations subcommand. Use: list, get, search, reply, close',
          }
      }
      break
    ...
    case 'messages':
      switch (sub) {
        case 'create': {
          ...
          if (!body || !adminId || !to) {
            result = {
              error: '--body, --admin-id, and --to (user ID) required',
            }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown messages subcommand. Use: create --body <text> --admin-id <id> --to <user_id> [--type inapp|email]',
          }
      }
      break
    ...
    case 'companies':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!companyId) {
            result = { error: '--company-id required' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown companies subcommand. Use: list, get, create, update',
          }
      }
      break
    ...
    case 'tags':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown tags subcommand. Use: list, create, delete',
          }
      }
      break
    ...
    case 'articles':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!title || !authorId) {
            result = { error: '--title and --author-id required' }
            break
          }
          const body = {
            title,
        ...
        default:
          result = {
            error:
              'Unknown articles subcommand. Use: list, get, create, update, delete',
          }
      }
      break
    ...
    case 'events':
      switch (sub) {
        case 'create': {
          ...
          if (!eventName || !userId) {
            result = { error: '--name and --user-id required' }
            break
          }
          const body = {
    ...
    default:
      result = {
        ...
        usage: {
          contacts:
            'contacts [list | get --id <id> | create --email <email> | update --id <id> | search --field <f> --value <v> | delete --id <id> | tag --id <id> --tag-id <id> | untag --id <id> --tag-id <id>]',
          conversations:
            'conversations [list | get --id <id> | search --field <f> --value <v> | reply --id <id> --body <text> --admin-id <id> | close --id <id> --admin-id <id>]',
          messages:
            'messages [create --body <text> --admin-id <id> --to <user_id>]',
          companies:
            'companies [list | get --id <id> | create --company-id <id> --name <name> | update --id <id>]',
          tags: 'tags [list | create --name <name> | delete --id <id>]',
          articles:
            'articles [list | get --id <id> | create --title <title> --author-id <id> | update --id <id> | delete --id <id>]',
          admins: 'admins [list | get --id <id>]',
          events:
            'events [create --name <name> --user-id <id> | list --user-id <id>]',

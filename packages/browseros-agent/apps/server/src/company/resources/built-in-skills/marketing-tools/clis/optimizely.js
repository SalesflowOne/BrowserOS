...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'OPTIMIZELY_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'projects':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown projects subcommand. Use: list, get, create',
          }
      }
      break
    ...
    case 'experiments':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (args['traffic-allocation'])
            body.traffic_allocation = Number(args['traffic-allocation'])
        ...
        case 'archive': {
          ...
          result = await api('PATCH', `/experiments/${id}`, {
            status: 'archived',
          })
          break
        }
        default:
          result = {
            error:
              'Unknown experiments subcommand. Use: list, get, create, update, results, archive',
          }
      }
      break
    ...
    case 'campaigns':
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/campaigns?project_id=${projectId}&page=${page}&per_page=${perPage}`,
          )
          break
        }
        case 'get': {
        ...
        default:
          result = {
            error: 'Unknown campaigns subcommand. Use: list, get, results',
          }
      }
      break
    ...
    case 'audiences':
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/audiences?project_id=${projectId}&page=${page}&per_page=${perPage}`,
          )
          break
        }
        case 'get': {
    ...
    case 'events':
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/events?project_id=${projectId}&page=${page}&per_page=${perPage}`,
          )
          break
        }
        default:
    ...
    case 'pages':
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/pages?project_id=${projectId}&page=${page}&per_page=${perPage}`,
          )
          break
        }
        default:
    ...
    default:
      result = {
        ...
        usage: {
          ...
          experiments:
            'experiments [list --project-id <id> | get --id <id> | create --project-id <id> --name <name> | update --id <id> --status <status> | results --id <id> | archive --id <id>]',
          campaigns:
            'campaigns [list --project-id <id> | get --id <id> | results --id <id>]',
          audiences: 'audiences [list --project-id <id> | get --id <id>]',
          events: 'events list --project-id <id>',
          pages: 'pages list --project-id <id>',
          options:
            '--page <n> --per-page <n> --status <not_started|running|paused|archived>',
        },
      }
  }

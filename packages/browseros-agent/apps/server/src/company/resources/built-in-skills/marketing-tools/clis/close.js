...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'CLOSE_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    ...
    case 'contacts':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown contacts subcommand. Use: list, get, create',
          }
      }
      break
    ...
    case 'opportunities':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!value) {
            result = { error: '--value required (in cents)' }
            break
          }
        ...
        default:
          result = {
            error: 'Unknown opportunities subcommand. Use: list, get, create',
          }
      }
      break
    ...
    case 'tasks':
      switch (sub) {
        case 'list': {
          const params = new URLSearchParams()
          if (args['assigned-to'])
            params.set('assigned_to', args['assigned-to'])
          if (args['is-complete'])
            params.set('is_complete', args['is-complete'])
          const qs = params.toString()
      ...
      result = {
        ...
        usage: {
          leads: {
            ...
            get: 'leads get --id <id>',
            create:
              'leads create --name <name> [--url <url>] [--description <desc>]',
          },
          contacts: {
            list: 'contacts list [--lead-id <id>]',
            get: 'contacts get --id <id>',
            create:
              'contacts create --lead-id <id> --name <name> [--email <email>] [--phone <phone>]',
          },
          opportunities: {
            list: 'opportunities list [--status <status>]',
            get: 'opportunities get --id <id>',
            create:
              'opportunities create --lead-id <id> --value <cents> [--status <status>]',
          },
          activities: {
          ...
          tasks: {
            ...
            create:
              'tasks create --lead-id <id> --text <text> [--assigned-to <user-id>] [--date <date>]',
          },
        },
      }
  }

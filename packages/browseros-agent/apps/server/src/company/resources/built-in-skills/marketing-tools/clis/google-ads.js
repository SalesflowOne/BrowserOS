...
if (!TOKEN || !DEV_TOKEN || !CUSTOMER_ID) {
  console.error(
    JSON.stringify({
      error:
        'GOOGLE_ADS_TOKEN, GOOGLE_ADS_DEVELOPER_TOKEN, and GOOGLE_ADS_CUSTOMER_ID environment variables required',
    }),
  )
  process.exit(1)
}
...
async function gaql(query) {
  return api('POST', `/customers/${CUSTOMER_ID}/googleAds:searchStream`, {
    query,
  })
}
    ...
    case 'campaigns':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown campaigns subcommand. Use: list, performance, pause, enable',
          }
      }
      break
    ...
    case 'adgroups':
      switch (sub) {
        case 'performance': {
          ...
          result = await gaql(
            `SELECT ad_group.name, metrics.impressions, metrics.clicks, metrics.conversions FROM ad_group WHERE segments.date DURING ${dateRange}${limit}`,
          )
          break
        }
        default:
      ...
      switch (sub) {
        case 'update': {
          if (!args.id || !args.amount) {
            result = { error: '--id and --amount required' }
            break
          }
          const amountMicros = String(
            Math.round(parseFloat(args.amount) * 1000000),
          )
          result = await api(
            'POST',
    ...
    default:
      result = {
        ...
        usage: {
          ...
          campaigns:
            'campaigns [list|performance|pause|enable] [--days 30] [--id <id>]',

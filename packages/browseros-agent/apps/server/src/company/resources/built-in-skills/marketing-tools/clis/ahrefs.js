...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'AHREFS_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    ...
    case 'backlinks':
      switch (sub) {
        case 'list': {
          if (!args.target) {
            result = { error: '--target required (domain or URL)' }
            break
          }
    ...
    case 'keyword-suggestions':
      switch (sub) {
        case 'get': {
          ...
          result = await api(
            'GET',
            `/keywords-explorer/matching-terms?${params}`,
          )
          break
        }
        default:
    ...
    case 'serp':
      switch (sub) {
        case 'get': {
          ...
          result = await api(
            'GET',
            `/keywords-explorer/serp-overview?${params}`,
          )
          break
        }
        default:
    ...
    default:
      result = {
        ...
        usage: {
          ...
          backlinks:
            'backlinks list --target <domain> [--mode <mode>] [--limit <n>]',
          refdomains:
            'refdomains list --target <domain> [--mode <mode>] [--limit <n>]',
          keywords:
            'keywords organic --target <domain> [--country <cc>] [--limit <n>]',
          'top-pages':
            'top-pages list --target <domain> [--country <cc>] [--limit <n>]',
          'keyword-overview':
            'keyword-overview get --keywords <kw1,kw2> [--country <cc>]',
          'keyword-suggestions':
            'keyword-suggestions get --keyword <keyword> [--country <cc>] [--limit <n>]',

...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'SAVVYCAL_API_KEY environment variable required' }),
  )
  process.exit(1)
}
    ...
    case 'links':
      switch (sub) {
        ...
        case 'slots': {
          ...
          result = await api(
            'GET',
            `/scheduling-links/${id}/slots${qs ? '?' + qs : ''}`,
          )
          break
        }
        default:
          result = {
            error:
              'Unknown links subcommand. Use: list, get, create, update, delete, duplicate, toggle, slots',
          }
      }
      break
      ...
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!linkId || !startAt || !name || !email) {
            result = {
              error: '--link-id, --start-at, --name, and --email required',
            }
            break
          }
        ...
        default:
          result = {
            error: 'Unknown events subcommand. Use: list, get, create, cancel',
          }
      }
      break
    ...
    case 'webhooks':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!url || !events) {
            result = { error: '--url and --events (comma-separated) required' }
            break
          }
    ...
    default:
      result = {
        ...
        usage: {
          me: 'me',
          links:
            'links [list | get --id <id> | create --name <name> | update --id <id> | delete --id <id> | duplicate --id <id> | toggle --id <id> | slots --id <id>]',
          events:
            'events [list | get --id <id> | create --link-id <id> --start-at <iso> --name <name> --email <email> | cancel --id <id>]',
          webhooks:
            'webhooks [list | create --url <url> --events <e1,e2> | delete --id <id>]',

...
if (!REST_API_KEY) {
  console.error(
    JSON.stringify({
      error: 'ONESIGNAL_REST_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
...
if (!APP_ID) {
  console.error(
    JSON.stringify({ error: 'ONESIGNAL_APP_ID environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'notifications':
      switch (sub) {
        ...
        case 'list': {
          result = await api(
            'GET',
            `/api/v1/notifications?app_id=${APP_ID}&limit=${limit}&offset=${offset}`,
          )
          break
        }
        case 'get': {
          ...
          result = await api(
            'GET',
            `/api/v1/notifications/${id}?app_id=${APP_ID}`,
          )
          break
        }
        case 'cancel': {
        ...
        default:
          result = {
            error:
              'Unknown notifications subcommand. Use: send, list, get, cancel',
          }
      }
      break
    ...
    case 'segments':
      switch (sub) {
        case 'list': {
          result = await api(
            'GET',
            `/api/v1/apps/${APP_ID}/segments?offset=${offset}&limit=${limit}`,
          )
          break
        }
        case 'create': {
          ...
          try {
            ...
            result = { error: 'Invalid JSON in --filters' }
            break
          }
          result = await api('POST', `/api/v1/apps/${APP_ID}/segments`, {
            name,
            filters,
          })
          break
        ...
        default:
          result = {
            error: 'Unknown segments subcommand. Use: list, create, delete',
          }
      }
      break
    ...
    case 'users':
      switch (sub) {
        case 'get': {
          ...
          if (!aliasId) {
            result = { error: '--alias-id required' }
            break
          }
          result = await api(
            'GET',
            `/api/v1/apps/${APP_ID}/users/by/${aliasLabel}/${aliasId}`,
          )
          break
        }
        case 'create': {
          ...
          if (args.tags) {
            try {
              payload.tags = JSON.parse(args.tags)
            } catch {
              result = { error: 'Invalid --tags JSON' }
              break
            }
          }
        ...
        default:
          result = {
            error: 'Unknown users subcommand. Use: get, create, delete',
          }
      }
      break
    ...
    case 'templates':
      switch (sub) {
        case 'list': {
          result = await api(
            'GET',
            `/api/v1/templates?app_id=${APP_ID}&limit=${limit}&offset=${offset}`,
          )
          break
        }
        case 'get': {
        ...
        default:
          result = {
            error: 'Unknown templates subcommand. Use: list, get, create',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          notifications:
            'notifications [send --message <msg> --segment <s> | list | get --id <id> | cancel --id <id>]',
          segments:
            'segments [list | create --name <n> --filters <json> | delete --id <id>]',
          users:
            'users [get --alias-id <id> | create --external-id <id> --email <e> | delete --alias-id <id>]',
          templates:
            'templates [list | get --id <id> | create --name <n> --message <msg>]',
          app: 'app [get]',

...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'MAILCHIMP_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    ...
    case 'members':
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/lists/${args['list-id']}/members?${params}`,
          )
          break
        }
        case 'add': {
        ...
        case 'update': {
          ...
          result = await api(
            'PATCH',
            `/lists/${args['list-id']}/members/${subscriberHash}`,
            body,
          )
          break
        }
        default:
          result = {
            error: 'Unknown members subcommand. Use: list, add, update',
          }
      }
      break
    ...
    case 'campaigns':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown campaigns subcommand. Use: list, get, create, send',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          ...
          members:
            'members [list|add|update] [subscriber_hash] --list-id <id> [--email <email>] [--status <status>] [--first-name <name>] [--last-name <name>] [--tags <t1,t2>]',
          campaigns:
            'campaigns [list|get|create|send] [id] [--list-id <id>] [--subject <subject>] [--from-name <name>] [--reply-to <email>]',

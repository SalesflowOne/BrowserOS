...
if (!API_SECRET && !API_KEY) {
  console.error(
    JSON.stringify({
      error: 'KIT_API_SECRET or KIT_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'subscribers':
      switch (sub) {
        ...
        case 'get':
          if (!rest[0]) {
            result = { error: 'Subscriber ID required' }
            break
          }
        ...
        case 'update': {
          ...
          if (args.fields) {
            try {
              ...
              result = { error: 'Invalid JSON in --fields' }
              break
            }
          }
        ...
        default:
          result = {
            error:
              'Unknown subscribers subcommand. Use: list, get, update, unsubscribe',
          }
      }
      break
    ...
    case 'sequences':
      switch (sub) {
        ...
        case 'subscribe': {
          if (!rest[0]) {
            result = { error: 'Sequence ID required' }
            break
          }
          if (!args.email) {
            result = { error: '--email required' }
        ...
        default:
          result = {
            error: 'Unknown sequences subcommand. Use: list, subscribe',
          }
      }
      break
    ...
    case 'tags':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown tags subcommand. Use: list, subscribe, remove',
          }
      }
      break
      ...
      switch (sub) {
        ...
        case 'create': {
          if (!args.subject || !args.content) {
            result = { error: '--subject and --content required' }
            break
          }
          const body = {
          ...
          if (args['email-layout-template'])
            body.email_layout_template = args['email-layout-template']
    ...
    default:
      result = {
        ...
        usage: {
          subscribers:
            'subscribers [list|get|update|unsubscribe] [id] [--email <email>] [--first-name <name>] [--fields <json>] [--page <n>]',
          forms:
            'forms [list|subscribe] [form_id] [--email <email>] [--first-name <name>] [--fields <json>]',
          sequences:
            'sequences [list|subscribe] [sequence_id] [--email <email>] [--first-name <name>] [--fields <json>]',
          tags: 'tags [list|subscribe|remove] [tag_id] [subscriber_id] [--email <email>] [--subscriber-id <id>]',
          broadcasts:
            'broadcasts [list|create] [--subject <subject>] [--content <html>] [--email-layout-template <template>] [--page <n>]',
        },
      }
  }

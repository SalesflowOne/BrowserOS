  ...
  switch (command) {
    case 'stargazers': {
      ...
      if (!repo) {
        result = {
          error:
            'Usage: stargazers <owner/repo> [--limit N] [--target N] [--enrich] [--with-email] [--with-company] [--with-blog] [--format csv|json]',
        }
        break
      }
      ...
      const page = await paginate(
        `/repos/${repo.owner}/${repo.repo}/stargazers`,
        { limit },
      )
      if (page._dry_run || page.error) {
        result = page
        break
      }
      let users = page.data
    ...
    case 'forks': {
      ...
      if (!repo) {
        result = {
          error:
            'Usage: forks <owner/repo> [--limit N] [--target N] [--enrich] [--with-email] [--with-company] [--with-blog] [--format csv|json]',
        }
        break
      }
      const limit = args.limit ? parseInt(args.limit, 10) : undefined
      const target = args.target ? parseInt(args.target, 10) : undefined
      const page = await paginate(`/repos/${repo.owner}/${repo.repo}/forks`, {
        limit,
      })
      if (page._dry_run || page.error) {
        result = page
    ...
    case 'watchers': {
      ...
      if (!repo) {
        result = {
          error:
            'Usage: watchers <owner/repo> [--limit N] [--target N] [--enrich] [--with-email] [--with-company] [--with-blog] [--format csv|json]',
        }
        break
      }
      const limit = args.limit ? parseInt(args.limit, 10) : undefined
      const target = args.target ? parseInt(args.target, 10) : undefined
      const page = await paginate(
        `/repos/${repo.owner}/${repo.repo}/subscribers`,
        { limit },
      )
      if (page._dry_run || page.error) {
        result = page
    ...
    case 'user': {
      const login = rest[0]
      if (!login) {
        result = { error: 'Usage: user <username>' }
        break
      }
    ...
    default:
      result = {
        ...
        usage: {
          stargazers:
            'stargazers <owner/repo> [--limit N] [--target N] [--enrich] [--with-email] [--with-company] [--with-blog] [--type User|Organization] [--format csv|json]',
          forks:
            'forks <owner/repo> [--limit N] [--target N] [--enrich] [--with-email] [--with-company] [--with-blog] [--type User|Organization] [--format csv|json]',
          watchers:
            'watchers <owner/repo> [--limit N] [--target N] [--enrich] [--with-email] [--with-company] [--with-blog] [--format csv|json]',
          user: 'user <username>',

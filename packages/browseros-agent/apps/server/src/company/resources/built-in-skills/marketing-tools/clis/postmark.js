...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'POSTMARK_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'email':
      switch (sub) {
        case 'send': {
          ...
          if (!from) {
            result = { error: '--from required' }
            break
          }
          if (!to) {
            result = { error: '--to required' }
            break
          }
          if (!subject) {
            result = { error: '--subject required' }
            break
          }
          const body = {
            From: from,
        ...
        case 'send-template': {
          ...
          if (!template) {
            result = { error: '--template required (template ID or alias)' }
            break
          }
          const body = {
            From: from,
        ...
        case 'send-batch': {
          ...
          if (!from || !to || !subject) {
            result = {
              error: '--from, --to (comma-separated), and --subject required',
            }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown email subcommand. Use: send, send-template, send-batch',
          }
      }
      break
    ...
    case 'templates':
      switch (sub) {
        ...
        case 'get': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (template ID or alias)' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown templates subcommand. Use: list, get, create, delete',
          }
      }
      break
    ...
    case 'bounces':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown bounces subcommand. Use: list, get, stats, activate',
          }
      }
      break
    ...
    case 'messages':
      switch (sub) {
        ...
        case 'get': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (message ID)' }
            break
          }
        ...
        default:
          result = {
            error: 'Unknown messages subcommand. Use: outbound, inbound, get',
          }
      }
      break
    ...
    case 'stats':
      switch (sub) {
        ...
        case 'sends': {
          ...
          result = await api(
            'GET',
            `/stats/outbound/sends?${params.toString()}`,
          )
          break
        }
        case 'bounces': {
          ...
          result = await api(
            'GET',
            `/stats/outbound/bounces?${params.toString()}`,
          )
          break
        }
        case 'opens': {
          ...
          result = await api(
            'GET',
            `/stats/outbound/opens?${params.toString()}`,
          )
          break
        }
        case 'clicks': {
          ...
          result = await api(
            'GET',
            `/stats/outbound/clicks?${params.toString()}`,
          )
          break
        }
        case 'spam': {
        ...
        default:
          result = {
            error:
              'Unknown stats subcommand. Use: overview, sends, bounces, opens, clicks, spam',
          }
      }
      break
    ...
    case 'suppressions':
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/message-streams/${args.stream || 'outbound'}/suppressions/dump`,
          )
          break
        }
        case 'create': {
          ...
          result = await api(
            'POST',
            `/message-streams/${stream}/suppressions`,
            {
              Suppressions: email
                .split(',')
                .map((e) => ({ EmailAddress: e.trim() })),
            },
          )
          break
        }
        case 'delete': {
          ...
          result = await api(
            'POST',
            `/message-streams/${stream}/suppressions/delete`,
            {
              Suppressions: email
                .split(',')
                .map((e) => ({ EmailAddress: e.trim() })),
        ...
        default:
          result = {
            error: 'Unknown suppressions subcommand. Use: list, create, delete',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          email:
            'email [send --from <from> --to <to> --subject <subj> | send-template --from <from> --to <to> --template <id> | send-batch --from <from> --to <to1,to2> --subject <subj>]',
          templates:
            'templates [list | get --id <id> | create --name <name> | delete --id <id>]',
          bounces:
            'bounces [list | get --id <id> | stats | activate --id <id>]',
          messages: 'messages [outbound | inbound | get --id <id>]',
          stats: 'stats [overview | sends | bounces | opens | clicks | spam]',
          server: 'server [get]',
          suppressions:
            'suppressions [list | create --email <email> | delete --email <email>]',

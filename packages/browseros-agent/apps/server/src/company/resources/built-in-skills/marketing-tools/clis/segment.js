...
if (!WRITE_KEY && !ACCESS_TOKEN) {
  console.error(
    JSON.stringify({
      error:
        'SEGMENT_WRITE_KEY (for tracking) or SEGMENT_ACCESS_TOKEN (for profiles) environment variable required',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    ...
    case 'identify':
      switch (sub) {
        case 'user': {
          ...
          if (args.traits) {
            try {
              body.traits = JSON.parse(args.traits)
            } catch {
              result = { error: 'Invalid JSON in --traits' }
              break
            }
          }
    ...
    case 'batch':
      switch (sub) {
        case 'send': {
          if (!args.events) {
            result = { error: '--events required (JSON array)' }
            break
          }
          let batch
          try {
            batch = JSON.parse(args.events)
          } catch {
            result = { error: 'Invalid JSON in --events' }
            break
          }
    ...
    case 'profiles':
      switch (sub) {
        case 'traits': {
          if (!args['space-id']) {
            result = { error: '--space-id required' }
            break
          }
          if (!args['user-id']) {
            result = { error: '--user-id required' }
          ...
          result = await profileApi(
            'GET',
            `/spaces/${args['space-id']}/collections/users/profiles/user_id:${args['user-id']}/traits`,
          )
          break
        }
        case 'events': {
          ...
          result = await profileApi(
            'GET',
            `/spaces/${args['space-id']}/collections/users/profiles/user_id:${args['user-id']}/events`,
          )
          break
        }
        default:
    ...
    default:
      result = {
        ...
        usage: {
          track:
            'track event --user-id <id> --event <name> [--properties <json>]',

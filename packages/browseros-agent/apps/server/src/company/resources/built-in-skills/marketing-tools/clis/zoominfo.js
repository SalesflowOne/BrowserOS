...
if (!ACCESS_TOKEN && !process.env.ZOOMINFO_USERNAME) {
  console.error(
    JSON.stringify({
      error:
        'ZOOMINFO_ACCESS_TOKEN or ZOOMINFO_USERNAME + ZOOMINFO_PRIVATE_KEY environment variables required',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'auth': {
      ...
      if (args['show-token']) {
        ...
        const masked =
          token.length > 12 ? token.slice(0, 6) + '…' + token.slice(-6) : '***'
    ...
    case 'contacts':
      switch (sub) {
        case 'search': {
          ...
          if (args.location)
            (body.locationSearchType = 'PersonLocation'),
              (body.personLocationCity = [args.location])
    ...
    case 'companies':
      switch (sub) {
        case 'search': {
          ...
          if (args['employees-min'])
            body.employeeCountMin = Number(args['employees-min'])
          if (args['employees-max'])
            body.employeeCountMax = Number(args['employees-max'])
        ...
        default:
          result = {
            error: 'Unknown companies subcommand. Use: search, enrich',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          ...
          contacts: {
            search:
              'contacts search [--job-title <t>] [--company <c>] [--location <l>] [--seniority <s>] [--department <d>] [--page <n>]',
            enrich: 'contacts enrich --email <email> | --person-id <id>',
          },
          companies: {
            search:
              'companies search [--name <n>] [--industry <i>] [--revenue-min <n>] [--employees-min <n>] [--page <n>]',
          ...
          scoops: {
            search:
              'scoops search [--company-id <id>] [--topic <topic>] [--page <n>]',
          },
        },
      }
  }

...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'BEEHIIV_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'publications':
      switch (sub) {
        ...
        case 'get': {
          if (!pubId) {
            result = { error: '--publication required' }
            break
          }
    ...
    case 'subscriptions':
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/publications/${pubId}/subscriptions?${params.toString()}`,
          )
          break
        }
        case 'get': {
          ...
          result = await api(
            'GET',
            `/publications/${pubId}/subscriptions/${subId}`,
          )
          break
        }
        case 'create': {
          ...
          if (args['referring-site'])
            body.referring_site = args['referring-site']
          result = await api(
            'POST',
        ...
        default:
          result = {
            error:
              'Unknown subscriptions subcommand. Use: list, get, create, update, delete',
          }
      }
      break
    ...
    case 'posts':
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/publications/${pubId}/posts?${params.toString()}`,
          )
          break
        }
        case 'get': {
        ...
        default:
          result = {
            error: 'Unknown posts subcommand. Use: list, get, create, delete',
          }
      }
      break
    ...
    case 'automations':
      switch (sub) {
        ...
        case 'get': {
          ...
          result = await api(
            'GET',
            `/publications/${pubId}/automations/${autoId}`,
          )
          break
        }
        default:
    ...
    default:
      result = {
        ...
        usage: {
          ...
          subscriptions:
            'subscriptions [list | get --id <id> | create --email <email> | update --id <id> | delete --id <id>] --publication <id>',
          posts:
            'posts [list | get --id <id> | create --title <title> | delete --id <id>] --publication <id>',
          ...
          options:
            '--publication <id> --limit <n> --email <email> --status <status> --tier <tier>',
        },
      }
  }

...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'KEYWORDS_EVERYWHERE_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
    ...
    case 'keywords':
      switch (sub) {
        case 'data': {
          ...
          if (!kw) {
            result = {
              error: '--kw required (comma-separated keywords, max 100)',
            }
            break
          }
          result = await api('POST', '/get_keyword_data', {
            country,
            currency,
            dataSource,
            kw,
        ...
        case 'related': {
          const kw = args.kw?.split(',')
          if (!kw) {
            result = { error: '--kw required (comma-separated keywords)' }
            break
          }
          result = await api('POST', '/get_related_keywords', {
            country,
            currency,
            dataSource,
            kw,
        ...
        case 'pasf': {
          ...
          result = await api('POST', '/get_pasf_keywords', {
            country,
            currency,
            dataSource,
            kw,
        ...
        default:
          result = {
            error: 'Unknown keywords subcommand. Use: data, related, pasf',
          }
      }
      break
    ...
    case 'domain':
      switch (sub) {
        case 'keywords': {
          ...
          result = await api('POST', '/get_domain_keywords', {
            country,
            currency,
            domain,
          })
        ...
        default:
          result = {
            error:
              'Unknown domain subcommand. Use: keywords, traffic, backlinks, unique-backlinks',
          }
      }
      break
    ...
    case 'url':
      switch (sub) {
        case 'keywords': {
          ...
          result = await api('POST', '/get_url_keywords', {
            country,
            currency,
            url,
          })
        ...
        default:
          result = {
            error:
              'Unknown url subcommand. Use: keywords, traffic, backlinks, unique-backlinks',
          }
      }
      break
    ...
    case 'account':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown account subcommand. Use: credits, countries, currencies',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          ...
          domain:
            'domain [keywords|traffic|backlinks|unique-backlinks] --domain <domain>',

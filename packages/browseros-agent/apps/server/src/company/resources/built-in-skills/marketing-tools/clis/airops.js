...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'AIROPS_API_KEY environment variable required' }),
  )
  process.exit(1)
}
...
if (!WORKSPACE_ID) {
  console.error(
    JSON.stringify({
      error: 'AIROPS_WORKSPACE_ID environment variable required',
    }),
  )
  process.exit(1)
}
    ...
    case 'flows':
      switch (sub) {
        ...
        case 'execute': {
          ...
          result = await api(
            'POST',
            `/workspaces/${WORKSPACE_ID}/flows/${id}/execute`,
            { inputs: parsedInputs },
          )
          break
        }
        case 'runs': {
          ...
          result = await api(
            'GET',
            `/workspaces/${WORKSPACE_ID}/flows/${id}/runs`,
          )
          break
        }
        case 'run-status': {
        ...
        default:
          result = {
            error:
              'Unknown flows subcommand. Use: list, get, execute, runs, run-status',
          }
      }
      break
    ...
    case 'workflows':
      switch (sub) {
        ...
        case 'execute': {
          ...
          result = await api(
            'POST',
            `/workspaces/${WORKSPACE_ID}/workflows/${id}/execute`,
            { inputs: parsedInputs },
          )
          break
        }

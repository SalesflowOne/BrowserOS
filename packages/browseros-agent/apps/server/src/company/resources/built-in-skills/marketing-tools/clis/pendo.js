...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'PENDO_INTEGRATION_KEY environment variable required',
    }),
  )
  process.exit(1)
}
    ...
    case 'visitors':
      switch (sub) {
        ...
        case 'search': {
          ...
          if (!query) {
            result = { error: '--query <json> required' }
            break
          }
          let body
          try {
            ...
            result = { error: 'Invalid JSON in --query' }
            break
          }
    ...
    case 'reports':
      switch (sub) {
        case 'funnel': {
          ...
          if (!pipeline) {
            result = { error: '--pipeline <json> required' }
            break
          }
          let body
          try {
            body = JSON.parse(pipeline)
          } catch {
            result = { error: 'Invalid JSON in --pipeline' }
            break
          }
          result = await api('POST', '/aggregation', body)
          break
    ...
    case 'metadata':
      switch (sub) {
        case 'list': {
          ...
          if (!kind) {
            result = {
              error: '--kind <visitor|account|parentAccount> required',
            }
            break
          }

...
if (!hasTrackAuth && !hasAppAuth) {
  console.error(
    JSON.stringify({
      error:
        'CUSTOMERIO_APP_KEY (for App API) or CUSTOMERIO_SITE_ID + CUSTOMERIO_API_KEY (for Track API) environment variables required',
    }),
  )
  process.exit(1)
}

const basicAuth = hasTrackAuth
  ? Buffer.from(`${SITE_ID}:${API_KEY}`).toString('base64')
  : null
...
async function trackApi(method, path, body) {
  if (!hasTrackAuth) {
    return {
      error:
        'Track API requires CUSTOMERIO_SITE_ID and CUSTOMERIO_API_KEY environment variables',
    }
  }
  if (args['dry-run']) {
    return {
  ...
  switch (cmd) {
    case 'customers':
      switch (sub) {
        case 'identify': {
          ...
          if (!customerId) {
            result = { error: 'Customer ID required (positional arg or --id)' }
            break
          }
          const body = {}
        ...
        case 'track-event': {
          ...
          if (!args.name) {
            result = { error: '--name required (event name)' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown customers subcommand. Use: identify, get, delete, track-event',
          }
      }
      break
    ...
    case 'campaigns':
      switch (sub) {
        ...
        case 'get': {
          ...
          if (!campaignId) {
            result = { error: 'Campaign ID required (positional arg or --id)' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown campaigns subcommand. Use: list, get, metrics, trigger',
          }
      }
      break
    ...
    case 'send':
      switch (sub) {
        case 'email': {
          ...
          if (args['identifier-email'])
            body.identifiers.email = args['identifier-email']
    ...
    default:
      result = {
        ...
        usage: {
          customers:
            'customers [identify|get|delete|track-event] <customer_id> [--email <email>] [--first-name <name>] [--plan <plan>] [--data <json>] [--name <event>]',
          campaigns:
            'campaigns [list|get|metrics|trigger] [campaign_id] [--emails <e1,e2>] [--ids <id1,id2>] [--data <json>]',

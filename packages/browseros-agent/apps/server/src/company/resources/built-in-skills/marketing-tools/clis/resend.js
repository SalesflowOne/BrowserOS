...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'RESEND_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'send': {
      if (!args.from || !args.to || !args.subject) {
        result = { error: '--from, --to, and --subject required' }
        break
      }
      const body = {
        from: args.from,
      ...
      if (args.tags)
        body.tags = args.tags.split(',').map((t) => {
    ...
    case 'emails':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown emails subcommand. Use: list, get, cancel',
          }
      }
      break
    ...
    case 'domains':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown domains subcommand. Use: list, get, create, verify, delete',
          }
      }
      break
    ...
    case 'api-keys':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown api-keys subcommand. Use: list, create, delete',
          }
      }
      break
    ...
    case 'audiences':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown audiences subcommand. Use: list, get, create, delete',
          }
      }
      break
    ...
    case 'contacts': {
      ...
      if (!audienceId) {
        result = { error: 'Audience ID required as subcommand arg' }
        break
      }
      const action = rest[0]
      ...
      switch (action) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/audiences/${audienceId}/contacts?${params}`,
          )
          break
        }
        case 'get':
          if (!rest[1]) {
            result = { error: 'Contact ID required' }
            break
          }
          result = await api(
            'GET',
        ...
        case 'create': {
          ...
          if (args.unsubscribed)
            body.unsubscribed = args.unsubscribed === 'true'
        ...
        default:
          result = {
            error:
              'Unknown contacts action. Use: list, get, create, update, delete',
          }
      }
      break
    }
    ...
    case 'webhooks':
      switch (sub) {
        ...
        case 'create': {
          if (!args.url) {
            result = { error: '--url required (webhook URL)' }
            break
          }
    ...
    case 'batch': {
      ...
      try {
        ...
        result = { error: 'Invalid JSON for --emails: ' + e.message }
        break
      }
    ...
    case 'templates':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (args.variables) {
            try {
              body.variables = JSON.parse(args.variables)
            } catch (e) {
              result = { error: 'Invalid JSON for --variables: ' + e.message }
              break
            }
          }
        ...
        default:
          result = {
            error:
              'Unknown templates subcommand. Use: list, get, create, update, delete, publish, duplicate',
          }
      }
      break
    ...
    case 'broadcasts':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown broadcasts subcommand. Use: list, get, create, send, delete',
          }
      }
      break
    ...
    case 'segments':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown segments subcommand. Use: list, get, create, delete',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          ...
          domains:
            'domains [list|get|create|verify|delete] [id] [--name <name>]',
          'api-keys': 'api-keys [list|create|delete] [id] [--name <name>]',
          audiences: 'audiences [list|get|create|delete] [id] [--name <name>]',
          contacts:
            'contacts <audience_id> [list|get|create|update|delete] [contact_id] [--email <email>]',
          webhooks: 'webhooks [list|get|create|delete] [id] [--endpoint <url>]',
          batch: 'batch --emails <json_array>',
          templates:
            'templates [list|get|create|update|delete|publish|duplicate] [id] [--name <name>] [--html <html>] [--variables <json>]',
          broadcasts:
            'broadcasts [list|get|create|send|delete] [id] [--segment-id <id>] [--from <email>] [--subject <subject>]',

...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'ACTIVECAMPAIGN_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
...
if (!API_URL) {
  console.error(
    JSON.stringify({
      error:
        'ACTIVECAMPAIGN_API_URL environment variable required (e.g. https://yourname.api-us1.com)',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'contacts':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown contacts subcommand. Use: list, get, create, update, delete, sync',
          }
      }
      break
    ...
    case 'lists':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (args['sender-reminder'])
            list.sender_reminder = args['sender-reminder']
        ...
        default:
          result = {
            error:
              'Unknown lists subcommand. Use: list, get, create, delete, subscribe, unsubscribe',
          }
      }
      break
    ...
    case 'deals':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown deals subcommand. Use: list, get, create, update, delete',
          }
      }
      break
    ...
    case 'automations':
      switch (sub) {
        ...
        case 'add-contact': {
          ...
          const contactId = args['contact-id']
          if (!automationId) {
            result = { error: '--id required (automation ID)' }
            break
          }
          if (!contactId) {
            result = { error: '--contact-id required (contact ID, not email)' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown automations subcommand. Use: list, get, add-contact',
          }
      }
      break
    ...
    case 'tags':
      switch (sub) {
        ...
        case 'remove-from-contact': {
          ...
          if (!contactTagId) {
            result = { error: '--id required (contactTag ID)' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown tags subcommand. Use: list, get, create, delete, add-to-contact, remove-from-contact',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          contacts:
            'contacts [list | get --id <id> | create --email <email> | update --id <id> | delete --id <id> | sync --email <email>]',
          lists:
            'lists [list | get --id <id> | create --name <name> | delete --id <id> | subscribe --list-id <lid> --contact-id <cid> | unsubscribe --list-id <lid> --contact-id <cid>]',
          campaigns: 'campaigns [list | get --id <id>]',
          deals:
            'deals [list | get --id <id> | create --title <title> | update --id <id> | delete --id <id>]',
          automations:
            'automations [list | get --id <id> | add-contact --id <aid> --email <email>]',
          ...
          webhooks:
            'webhooks [list | get --id <id> | create --name <name> --url <url> | delete --id <id>]',
          users: 'users [me | list]',

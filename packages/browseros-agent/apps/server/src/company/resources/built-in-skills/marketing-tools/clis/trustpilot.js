...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'TRUSTPILOT_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
...
async function getAccessToken() {
  ...
  const res = await fetch(
    `${BASE_URL}/oauth/oauth-business-users-for-applications/accesstoken`,
    {
      method: 'POST',
      headers: {
        Authorization:
...
async function api(method, path, body, auth = 'apikey') {
  ...
  if (auth === 'bearer') {
    ...
    if (!token) {
      return {
        error: 'TRUSTPILOT_API_SECRET required for private API endpoints',
      }
    }
  ...
  switch (cmd) {
    case 'business':
      switch (sub) {
        case 'search': {
          ...
          result = await api(
            'GET',
            `/business-units/search?query=${encodeURIComponent(query)}&limit=${limit}`,
          )
          break
        }
        case 'get': {
          if (!businessUnitId) {
            result = {
              error: '--business-unit or TRUSTPILOT_BUSINESS_UNIT_ID required',
            }
            break
          }
        ...
        case 'profile': {
          ...
          result = await api(
            'GET',
            `/business-units/${businessUnitId}/profileinfo`,
          )
          break
        }
        case 'categories': {
        ...
        case 'web-links': {
          ...
          result = await api(
            'GET',
            `/business-units/${businessUnitId}/web-links?locale=${encodeURIComponent(locale)}`,
          )
          break
        }
        default:
          result = {
            error:
              'Unknown business subcommand. Use: search, get, profile, categories, web-links',
          }
      }
      break
    ...
    case 'reviews':
      switch (sub) {
        ...
        case 'private': {
          ...
          result = await api(
            'GET',
            `/private/business-units/${businessUnitId}/reviews?${privateParams}`,
            null,
            'bearer',
          )
          break
        ...
        case 'reply': {
          ...
          result = await api(
            'POST',
            `/private/reviews/${reviewId}/reply`,
            { message },
            'bearer',
          )
          break
        ...
        default:
          result = {
            error:
              'Unknown reviews subcommand. Use: list, get, private, latest, reply, delete-reply',
          }
      }
      break
    ...
    case 'invitations':
      switch (sub) {
        ...
        case 'templates': {
          ...
          result = await api(
            'GET',
            `/private/business-units/${businessUnitId}/templates`,
            null,
            'bearer',
          )
          break
        ...
        default:
          result = {
            error:
              'Unknown invitations subcommand. Use: create, link, templates',
          }
      }
      break
    ...
    case 'tags':
      switch (sub) {
        ...
        case 'add': {
          ...
          if (!group || !value) {
            result = { error: '--group and --value required' }
            break
          }
          result = await api(
            'PUT',
        ...
        case 'remove': {
          ...
          result = await api(
            'DELETE',
            `/private/reviews/${reviewId}/tags?group=${encodeURIComponent(group)}&value=${encodeURIComponent(value)}`,
            null,
            'bearer',
          )
          break
    ...
    default:
      result = {
        ...
        usage: {
          business:
            'business [search --query <q> | get | profile | categories | web-links]',
          reviews:
            'reviews [list | get --id <id> | private | latest | reply --id <id> --message <msg> | delete-reply --id <id>]',
          invitations:
            'invitations [create --email <e> --name <n> | link --email <e> --name <n> | templates]',
          tags: 'tags [get --id <id> | add --id <id> --group <g> --value <v> | remove --id <id> --group <g> --value <v>]',
          options:
            '--business-unit <id> --limit <n> --stars <1-5> --language <code>',
        },
      }
  }

...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'CLAY_API_KEY environment variable required' }),
  )
  process.exit(1)
}
    ...
    case 'tables':
      switch (sub) {
        ...
        case 'rows': {
          ...
          result = await api(
            'GET',
            `/tables/${id}/rows?page=${page}&per_page=${perPage}`,
          )
          break
        }
        case 'add-row': {
          ...
          if (!data) {
            result = { error: '--data required (JSON string)' }
            break
          }
          let parsed
          try {
        ...
        default:
          result = {
            error: 'Unknown tables subcommand. Use: list, get, rows, add-row',
          }
      }
      break
    ...
    case 'people':
      switch (sub) {
        case 'enrich': {
          ...
          if (args['first-name'] && args['last-name'] && args.domain)
            body.domain = args.domain
          if (
            !args.email &&
            !args.linkedin &&
            !(args['first-name'] && args['last-name'] && args.domain)
          ) {
            result = {
              error:
                '--email or --linkedin required (or --first-name + --last-name + --domain)',
            }
            break
          }
    ...
    default:
      result = {
        ...
        usage: {
          ...
          people: {
            enrich:
              'people enrich --email <email> | --linkedin <url> | --first-name <n> --last-name <n> --domain <d>',
          },
          companies: {

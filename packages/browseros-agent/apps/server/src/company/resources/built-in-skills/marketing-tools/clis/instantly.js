...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'INSTANTLY_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
    ...
    case 'campaigns':
      switch (sub) {
        ...
        case 'launch': {
          ...
          result = await api('POST', '/campaign/launch', {
            api_key: API_KEY,
            campaign_id: id,
          })
          break
        ...
        case 'pause': {
          ...
          result = await api('POST', '/campaign/pause', {
            api_key: API_KEY,
            campaign_id: id,
          })
          break
        ...
        default:
          result = {
            error:
              'Unknown campaigns subcommand. Use: list, get, status, launch, pause',
          }
      }
      break
    ...
    case 'leads':
      switch (sub) {
        ...
        case 'add': {
          ...
          result = await api('POST', '/lead/add', {
            api_key: API_KEY,
        ...
        case 'delete': {
          ...
          result = await api('POST', '/lead/delete', {
            api_key: API_KEY,
            campaign_id: campaignId,
            delete_list: [email],
          })
        ...
        default:
          result = {
            error: 'Unknown leads subcommand. Use: list, add, delete, status',
          }
      }
      break
    ...
    case 'accounts':
      switch (sub) {
        ...
        case 'status': {
          ...
          if (!accountId) {
            result = { error: '--account-id required' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown accounts subcommand. Use: list, status, warmup-status',
          }
      }
      break
    ...
    case 'analytics':
      switch (sub) {
        ...
        case 'account': {
          ...
          if (!startDate) {
            result = { error: '--start-date required' }
            break
          }
          if (!endDate) {
            result = { error: '--end-date required' }
            break
          }
          result = await api('POST', '/analytics/campaign/count', {
            api_key: API_KEY,
        ...
        default:
          result = {
            error:
              'Unknown analytics subcommand. Use: campaign, steps, account',
          }
      }
      break
    ...
    case 'blocklist':
      switch (sub) {
        ...
        case 'add': {
          ...
          if (!entries) {
            result = {
              error: '--entries required (comma-separated emails or domains)',
            }
            break
          }
          const entryList = entries.split(',').map((e) => e.trim())
          result = await api('POST', '/blocklist/add', {
            api_key: API_KEY,
            entries: entryList,
          })
          break
    ...
    default:
      result = {
        ...
        usage: {
          ...
          analytics: {
            campaign:
              'analytics campaign --campaign-id <id> [--start-date YYYY-MM-DD] [--end-date YYYY-MM-DD]',
            steps:
              'analytics steps --campaign-id <id> [--start-date YYYY-MM-DD] [--end-date YYYY-MM-DD]',
            account:
              'analytics account --start-date YYYY-MM-DD --end-date YYYY-MM-DD',
          },
          blocklist: {
            list: 'blocklist list',

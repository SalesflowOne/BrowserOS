...
if (!TOKEN) {
  console.error(
    JSON.stringify({
      error: 'LINKEDIN_ACCESS_TOKEN environment variable required',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    ...
    case 'campaigns':
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/adCampaignsV2?q=search&search.account.values[0]=urn:li:sponsoredAccount:${args['account-id']}`,
          )
          break
        }
        case 'create': {
          if (!args['account-id'] || !args.name) {
            result = { error: '--account-id and --name required' }
            break
          }
          if (!args['campaign-group-id']) {
            result = { error: '--campaign-group-id required' }
            break
          }
          const body = {
        ...
        case 'analytics': {
          ...
          if (
            ...
            result = {
              error:
                '--start-year, --start-month, --start-day, --end-year, --end-month, --end-day required',
            }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown campaigns subcommand. Use: list, create, update, analytics',
          }
      }
      break
    ...
    case 'creatives':
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/adCreativesV2?q=search&search.campaign.values[0]=urn:li:sponsoredCampaign:${args['campaign-id']}`,
          )
          break
        }
        default:
    ...
    case 'audiences':
      switch (sub) {
        case 'count': {
          if (!args.targeting) {
            result = { error: '--targeting required (JSON string)' }
            break
          }
          let targeting
          try {
          ...
          result = await api('POST', '/audienceCountsV2', {
    ...
    default:
      result = {
        ...
        usage: {
          ...
          campaigns:
            'campaigns [list|create|update|analytics] [--account-id <id>] [--name <name>] [--type SPONSORED_UPDATES] [--cost-type CPC] [--unit-cost 5.00] [--daily-budget 100.00] [--id <id>] [--status ACTIVE|PAUSED]',

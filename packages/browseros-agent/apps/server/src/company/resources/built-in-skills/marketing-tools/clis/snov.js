...
if (!CLIENT_ID || !CLIENT_SECRET) {
  console.error(
    JSON.stringify({
      error:
        'SNOV_CLIENT_ID and SNOV_CLIENT_SECRET environment variables required',
    }),
  )
  process.exit(1)
}
      ...
      switch (sub) {
        case 'find': {
          ...
          if (!domain || !firstName || !lastName) {
            result = {
              error: '--domain, --first-name, and --last-name required',
            }
            break
          }
          result = await api('POST', '/get-emails-from-names', {
            firstName,
            lastName,
            domain,
          })
        ...
        case 'verify': {
          ...
          result = await api('POST', '/get-emails-verification-status', {
            emails: [email],
          })
          break
        }
    ...
    case 'drips':
      switch (sub) {
        ...
        case 'get': {
          ...
          result = await api(
            'GET',
            `/get-emails-from-campaign?id=${encodeURIComponent(id)}`,
          )
          break
        }
        case 'add-prospect': {
          ...
          const email = args.email
          if (!campaignId || !email) {
            result = { error: '--campaign-id and --email required' }
            break
          }
        ...
        default:
          result = {
            error: 'Unknown drips subcommand. Use: list, get, add-prospect',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          domain: {
            search:
              'domain search --domain <domain> [--type all|personal|generic] [--limit <n>]',
          ...
          lists: {
            list: 'lists list',
            prospects:
              'lists prospects --id <id> [--page <n>] [--per-page <n>]',
          },
          ...
          drips: {
            ...
            get: 'drips get --id <id>',
            'add-prospect':
              'drips add-prospect --campaign-id <id> --email <email> [--first-name <name>] [--last-name <name>]',
          },
        },
      }
  }

...
if (!TOKEN && !API_KEY) {
  console.error(
    JSON.stringify({
      error:
        'MIXPANEL_TOKEN (for ingestion) or MIXPANEL_API_KEY + MIXPANEL_SECRET (for query/export) environment variables required',
    }),
  )
  process.exit(1)
}
...
async function ingestApi(method, path, body) {
  ...
  if (args['dry-run']) {
    ...
    if (Array.isArray(maskedBody))
      maskedBody.forEach((item) => {
        if (item.properties && item.properties.token)
...
async function queryApi(method, baseUrl, path, params) {
  if (!API_KEY || !SECRET) {
    return {
      error:
        'MIXPANEL_API_KEY and MIXPANEL_SECRET required for query/export operations',
    }
  }
  ...
  switch (cmd) {
    case 'track':
      switch (sub) {
        case 'event': {
          if (!TOKEN) {
            result = { error: 'MIXPANEL_TOKEN required for tracking' }
            break
          }
          if (!args['distinct-id']) {
            result = { error: '--distinct-id required' }
            break
          }
          if (!args.event) {
          ...
          try {
            ...
            result = { error: 'Invalid JSON in --properties' }
            break
          }
    ...
    case 'profiles':
      switch (sub) {
        case 'set': {
          if (!TOKEN) {
            result = { error: 'MIXPANEL_TOKEN required for profiles' }
            break
          }
          if (!args['distinct-id']) {
            result = { error: '--distinct-id required' }
    ...
    case 'query':
      switch (sub) {
        case 'events': {
          if (!args['project-id']) {
            result = { error: '--project-id required' }
            break
          }
          const body = {
              ...
              time_range: {
                from_date:
                  ...
                  new Date(Date.now() - 30 * 86400000)
                    ...
                    .slice(0, 10),
                to_date:
                  args['to-date'] || new Date().toISOString().slice(0, 10),
              },
            },
          }
    ...
    case 'funnels':
      switch (sub) {
        case 'get': {
          if (!args['funnel-id']) {
            result = { error: '--funnel-id required' }
            break
          }
    ...
    case 'retention':
      switch (sub) {
        case 'get': {
          if (!args['from-date'] || !args['to-date']) {
            result = {
              error: '--from-date and --to-date required (YYYY-MM-DD)',
            }
            break
          }
          const params = new URLSearchParams()
    ...
    case 'export':
      switch (sub) {
        case 'events': {
          if (!args['from-date']) {
            result = { error: '--from-date required' }
            break
          }
          if (!args['to-date']) {
            result = { error: '--to-date required' }
            break
          }
          const params = new URLSearchParams()
          params.set('from_date', args['from-date'])
    ...
    default:
      result = {
        ...
        usage: {
          track:
            'track event --distinct-id <id> --event <name> [--properties <json>]',
          profiles: 'profiles set --distinct-id <id> [--properties <json>]',
          query:
            'query events --project-id <id> [--event <name>] [--from-date <date>] [--to-date <date>]',
          funnels:
            'funnels get --funnel-id <id> [--from-date <date>] [--to-date <date>]',
          retention:
            'retention get [--from-date <date>] [--to-date <date>] [--born-event <event>]',

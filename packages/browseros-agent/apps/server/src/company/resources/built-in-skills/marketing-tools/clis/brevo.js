...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'BREVO_API_KEY environment variable required' }),
  )
  process.exit(1)
}
    ...
    case 'contacts':
      switch (sub) {
        ...
        case 'get': {
          ...
          if (!id) {
            result = { error: '--id or --email required' }
            break
          }
        ...
        case 'create': {
          ...
          if (args['first-name'] || args['last-name']) {
            body.attributes = {}
            if (args['first-name'])
              body.attributes.FIRSTNAME = args['first-name']
            if (args['last-name']) body.attributes.LASTNAME = args['last-name']
          }
          if (args['list-ids'])
            body.listIds = args['list-ids'].split(',').map(Number)
        ...
        case 'update': {
          ...
          if (args['list-ids'])
            body.listIds = args['list-ids'].split(',').map(Number)
          if (args['unlink-list-ids'])
            body.unlinkListIds = args['unlink-list-ids'].split(',').map(Number)
        ...
        case 'import': {
          ...
          if (!emails) {
            result = { error: '--emails required (comma-separated)' }
            break
          }
          const body = {
        ...
        default:
          result = {
            error:
              'Unknown contacts subcommand. Use: list, get, create, update, delete, import',
          }
      }
      break
    ...
    case 'lists':
      switch (sub) {
        ...
        case 'contacts': {
          ...
          result = await api(
            'GET',
            `/contacts/lists/${id}/contacts?${params.toString()}`,
          )
          break
        }
        case 'add-contacts': {
          ...
          result = await api('POST', `/contacts/lists/${id}/contacts/add`, {
            emails,
          })
          break
        }
        case 'remove-contacts': {
          ...
          result = await api('POST', `/contacts/lists/${id}/contacts/remove`, {
            emails,
          })
          break
        }
        default:
          result = {
            error:
              'Unknown lists subcommand. Use: list, get, create, update, delete, contacts, add-contacts, remove-contacts',
          }
      }
      break
    ...
    case 'campaigns':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (args['list-ids'])
            body.recipients = {
              listIds: args['list-ids'].split(',').map(Number),
            }
        ...
        case 'send-test': {
          ...
          result = await api('POST', `/emailCampaigns/${id}/sendTest`, {
            emailTo: emails,
          })
          break
        }
        default:
          result = {
            error:
              'Unknown campaigns subcommand. Use: list, get, create, update, delete, send-now, send-test',
          }
      }
      break
    ...
    case 'sms':
      switch (sub) {
        case 'send': {
          ...
          if (!sender) {
            result = { error: '--from required (sender name)' }
            break
          }
          if (!recipient) {
            result = { error: '--to required (phone number)' }
            break
          }
          if (!content) {
    ...
    default:
      result = {
        ...
        usage: {
          account: 'account [get]',
          contacts:
            'contacts [list | get --email <email> | create --email <email> | update --email <email> | delete --email <email> | import --emails <e1,e2>]',
          lists:
            'lists [list | get --id <id> | create --name <name> | delete --id <id> | contacts --id <id> | add-contacts --id <id> --emails <e1,e2> | remove-contacts --id <id> --emails <e1,e2>]',
          email: 'email [send --from <from> --to <to> --subject <subj>]',
          campaigns:
            'campaigns [list | get --id <id> | create --name <name> | send-now --id <id> | send-test --id <id> --emails <e1,e2>]',

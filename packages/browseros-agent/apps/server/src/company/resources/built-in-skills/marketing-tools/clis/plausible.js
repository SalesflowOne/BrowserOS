...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'PLAUSIBLE_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
...
async function main() {
  ...
  switch (cmd) {
    case 'stats':
      if (!siteId) {
        result = { error: '--site-id required (your domain, e.g. example.com)' }
        break
      }
      switch (sub) {
        case 'aggregate': {
        ...
        case 'query': {
          const metrics = args.metrics?.split(',')
          if (!metrics) {
            result = { error: '--metrics required (comma-separated)' }
            break
          }
          ...
          if (args.filters) {
            try {
              body.filters = JSON.parse(args.filters)
            } catch {
              result = { error: '--filters must be valid JSON' }
              break
            }
          }
        ...
        case 'realtime':
          result = await api(
            'GET',
            `/api/v1/stats/realtime/visitors?site_id=${encodeURIComponent(siteId)}`,
          )
          break
        default:
          result = {
            error:
              'Unknown stats subcommand. Use: aggregate, timeseries, pages, sources, countries, devices, utm, query, realtime',
          }
      }
      break
    ...
    case 'sites':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown sites subcommand. Use: list, get, create, delete',
          }
      }
      break
    ...
    case 'goals':
      ...
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!goalType) {
            result = { error: '--goal-type required (event or page)' }
            break
          }
          const body = { site_id: siteId, goal_type: goalType }
          if (goalType === 'event') {
            if (!args['event-name']) {
              result = { error: '--event-name required for event goals' }
              break
            }
            body.event_name = args['event-name']
          } else if (goalType === 'page') {
            if (!args['page-path']) {
              result = { error: '--page-path required for page goals' }
              break
            }
        ...
        case 'delete': {
          ...
          if (!goalId) {
            result = { error: '--goal-id required' }
            break
          }
          result = await api('DELETE', `/api/v1/sites/goals/${goalId}`, {
            site_id: siteId,
          })
          break
        }
        default:
          result = {
            error: 'Unknown goals subcommand. Use: list, create, delete',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          stats: {
            aggregate:
              'stats aggregate --site-id <domain> [--date-range <30d>] [--metrics <m1,m2>]',
            timeseries:
              'stats timeseries --site-id <domain> [--date-range <30d>] [--period <time:day>]',
            pages:
              'stats pages --site-id <domain> [--date-range <30d>] [--limit <n>]',
            sources: 'stats sources --site-id <domain> [--date-range <30d>]',
            countries:
              'stats countries --site-id <domain> [--date-range <30d>]',
            devices: 'stats devices --site-id <domain> [--date-range <30d>]',
            utm: 'stats utm --site-id <domain> [--param <utm_source>] [--date-range <30d>]',
            query:
              'stats query --site-id <domain> --metrics <m1,m2> [--dimensions <d1,d2>] [--filters <json>]',
            realtime: 'stats realtime --site-id <domain>',
          },
          sites:
            'sites [list | get --site-id <domain> | create --domain <domain> | delete --site-id <domain>]',
          goals:
            'goals [list | create --goal-type <event|page> --event-name <name> | delete --goal-id <id>] --site-id <domain>',

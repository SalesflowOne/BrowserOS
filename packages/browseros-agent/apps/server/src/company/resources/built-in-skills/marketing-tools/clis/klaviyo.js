...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'KLAVIYO_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'profiles':
      switch (sub) {
        case 'list': {
          ...
          if (args['page-cursor'])
            params.set('page[cursor]', args['page-cursor'])
        ...
        default:
          result = {
            error:
              'Unknown profiles subcommand. Use: list, get, create, update',
          }
      }
      break
    ...
    case 'lists':
      switch (sub) {
        ...
        case 'add-profiles': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (list ID)' }
            break
          }
          const profileIds = args.profiles?.split(',')
          if (!profileIds) {
            result = {
              error: '--profiles required (comma-separated profile IDs)',
            }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown lists subcommand. Use: list, get, create, delete, add-profiles, remove-profiles',
          }
      }
      break
    ...
    case 'events':
      switch (sub) {
        ...
        case 'create': {
          ...
          const email = args.email
          if (!metric) {
            result = { error: '--metric required (metric name)' }
            break
          }
          if (!email) {
            result = { error: '--email required' }
        ...
        default:
          result = {
            error: 'Unknown events subcommand. Use: list, get, create',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          profiles:
            'profiles [list | get --id <id> | create --email <email> | update --id <id>]',
          lists:
            'lists [list | get --id <id> | create --name <name> | delete --id <id> | add-profiles --id <list-id> --profiles <id1,id2> | remove-profiles --id <list-id> --profiles <id1,id2>]',
          events:
            'events [list | get --id <id> | create --metric <name> --email <email>]',
          campaigns: 'campaigns [list | get --id <id>]',
          flows:
            'flows [list | get --id <id> | update --id <id> --status <status>]',

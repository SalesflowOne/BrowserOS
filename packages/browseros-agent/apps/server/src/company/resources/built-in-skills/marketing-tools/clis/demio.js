...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'DEMIO_API_KEY environment variable required' }),
  )
  process.exit(1)
}
...
if (!API_SECRET) {
  console.error(
    JSON.stringify({ error: 'DEMIO_API_SECRET environment variable required' }),
  )
  process.exit(1)
}
    ...
    case 'register':
      switch (sub) {
        case 'create': {
          ...
          if (!id) {
            result = { error: '--id (event id) required' }
            break
          }
          if (!name) {
    ...
    default:
      result = {
        ...
        usage: {
          ping: 'ping',
          events:
            'events [list --type <upcoming|past|all> | get --id <id> | date --event-id <id> --date-id <id>]',
          register:
            'register [create --id <event_id> --name <name> --email <email> --date-id <date_id>]',

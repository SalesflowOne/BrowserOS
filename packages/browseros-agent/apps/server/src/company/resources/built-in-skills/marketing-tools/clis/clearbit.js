...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'CLEARBIT_API_KEY environment variable required' }),
  )
  process.exit(1)
}
      ...
      switch (sub) {
        case 'find': {
          ...
          result = await api(
            ...
            `/v2/people/find?email=${encodeURIComponent(email)}`,
          )
          break
        }
        default:
    ...
    case 'company':
      switch (sub) {
        case 'find': {
          ...
          result = await api(
            'GET',
            'https://company-stream.clearbit.com',
    ...
    case 'reveal':
      switch (sub) {
        case 'find': {
          const ip = args.ip
          if (!ip) {
            result = { error: '--ip required' }
            break
          }
          result = await api(
            'GET',
            'https://reveal.clearbit.com',
            `/v1/companies/find?ip=${encodeURIComponent(ip)}`,
          )
          break
        }
        default:
      ...
      switch (sub) {
        case 'find': {
          ...
          result = await api(
            ...
            `/v1/domains/find?name=${encodeURIComponent(name)}`,
          )
          break
        }
        default:
    ...
    default:
      result = {
        ...
        usage: {
          ...
          prospector:
            'prospector search --domain <domain> [--role <role>] [--seniority <level>] [--title <title>]',
        },
      }
  }

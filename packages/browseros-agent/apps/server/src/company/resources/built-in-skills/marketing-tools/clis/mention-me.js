...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'MENTIONME_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'offers':
      switch (sub) {
        case 'create': {
          ...
          if (args['order-total'])
            body.order_total = Number(args['order-total'])
          if (args['order-currency'])
            body.order_currency = args['order-currency']
    ...
    case 'referrals':
      switch (sub) {
        case 'get': {
          ...
          if (!refId) {
            result = { error: 'Referral ID required (positional arg or --id)' }
            break
          }
        ...
        case 'list': {
          ...
          result = await api(
            'GET',
            `/referrer/${args['customer-id']}/referrals`,
          )
          break
        }
        default:
    ...
    case 'share-links':
      switch (sub) {
        case 'get':
          ...
          result = await api(
            'GET',
            `/referrer/${args['customer-id']}/share-links`,
          )
          break
        default:
    ...
    case 'rewards':
      switch (sub) {
        ...
        case 'redeem': {
          ...
          result = await api(
            'POST',
            `/referrer/${args['customer-id']}/rewards/redeem`,
            body,
          )
          break
        }
    ...
    default:
      result = {
        ...
        usage: {
          offers:
            'offers [create] [--email <email>] [--firstname <name>] [--lastname <name>] [--order-number <num>] [--order-total <total>] [--order-currency <currency>]',
          referrals: 'referrals [get|list] [id] [--customer-id <id>]',
          'share-links': 'share-links [get] [--customer-id <id>]',
          rewards:
            'rewards [get|redeem] [--customer-id <id>] [--reward-id <id>] [--order-number <num>]',
          referee:
            'referee [create] [--email <email>] [--firstname <name>] [--referrer-code <code>] [--order-number <num>] [--order-total <total>]',
        },
      }
  }

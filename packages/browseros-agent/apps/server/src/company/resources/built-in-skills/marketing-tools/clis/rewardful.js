...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'REWARDFUL_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'affiliates':
      switch (sub) {
        ...
        case 'get':
          if (!rest[0]) {
            result = { error: 'Affiliate ID required' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown affiliates subcommand. Use: list, get, search, update',
          }
      }
      break
      ...
      switch (sub) {
        ...
        case 'get': {
          ...
          if (args['stripe-customer-id'])
            params.set('stripe_customer_id', args['stripe-customer-id'])
          result = await api('GET', `/referrals?${params}`)
          break
        }
        default:
    ...
    case 'commissions':
      switch (sub) {
        ...
        case 'get':
          if (!rest[0]) {
            result = { error: 'Commission ID required' }
            break
          }
    ...
    case 'links':
      switch (sub) {
        case 'create': {
          ...
          result = await api(
            'POST',
            `/affiliates/${args['affiliate-id']}/links`,
            body,
          )
          break
        }
    ...
    default:
      result = {
        ...
        usage: {
          affiliates:
            'affiliates [list|get|search|update] [id] [--email <email>] [--id <id>] [--first-name <name>] [--last-name <name>] [--paypal-email <email>]',
          referrals:
            'referrals [list|get] [--affiliate-id <id>] [--stripe-customer-id <id>]',
          commissions: 'commissions [list|get] [id] [--affiliate-id <id>]',
          links:
            'links [create] [--affiliate-id <id>] [--token <token>] [--url <url>]',
        },
      }
  }

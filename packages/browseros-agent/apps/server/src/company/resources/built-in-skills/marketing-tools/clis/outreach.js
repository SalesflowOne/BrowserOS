...
if (!ACCESS_TOKEN) {
  console.error(
    JSON.stringify({
      error: 'OUTREACH_ACCESS_TOKEN environment variable required',
    }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'prospects':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown prospects subcommand. Use: list, get, create',
          }
      }
      break
    ...
    case 'sequence-states':
      switch (sub) {
        case 'create': {
          ...
          if (!prospectId) {
            result = { error: '--prospect-id required' }
            break
          }
          const body = {
            data: {
    ...
    case 'mailings':
      switch (sub) {
        case 'list': {
          const params = new URLSearchParams()
          if (args['sequence-id'])
            params.set('filter[sequence][id]', args['sequence-id'])
          const qs = params.toString()
      ...
      result = {
        ...
        usage: {
          prospects: {
            ...
            create:
              'prospects create --email <email> [--first-name <name>] [--last-name <name>]',
          },
          sequences: {
            list: 'sequences list',
          ...
          'sequence-states': {
            create:
              'sequence-states create --sequence-id <id> --prospect-id <id>',
          },
          mailings: {

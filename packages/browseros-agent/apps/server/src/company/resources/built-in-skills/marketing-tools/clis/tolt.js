...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'TOLT_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'affiliates':
      switch (sub) {
        ...
        case 'get': {
          ...
          if (!id) {
            result = { error: 'Affiliate ID required (positional arg or --id)' }
            break
          }
        ...
        case 'update': {
          if (!args.id) {
            result = { error: '--id required (affiliate ID)' }
            break
          }
          const body = {}
          if (args['commission-rate'])
            body.commission_rate = Number(args['commission-rate'])
        ...
        default:
          result = {
            error:
              'Unknown affiliates subcommand. Use: list, get, create, update',
          }
      }
      break
    ...
    case 'referrals':
      switch (sub) {
        case 'list': {
          ...
          if (args['affiliate-id'])
            params.set('affiliate_id', args['affiliate-id'])
        ...
        case 'get': {
          const params = new URLSearchParams()
          if (args['customer-id'])
            params.set('customer_id', args['customer-id'])
          result = await api('GET', `/referrals?${params}`)
          break
        }
        default:
    ...
    default:
      result = {
        ...
        usage: {
          affiliates:
            'affiliates [list|get|create|update] [id] [--email <email>] [--name <name>] [--id <id>] [--commission-rate <rate>] [--payout-method <method>] [--paypal-email <email>]',
          referrals:
            'referrals [list|get] [--affiliate-id <id>] [--customer-id <id>]',

...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'PADDLE_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
      ...
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!taxCategory) {
            result = {
              error:
                '--tax-category required (e.g. standard, digital-goods, saas)',
            }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown products subcommand. Use: list, get, create, update',
          }
      }
      break
    ...
    case 'prices':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!amount) {
            result = {
              error: '--amount required (in lowest denomination, e.g. cents)',
            }
            break
          }
          const body = {
        ...
        default:
          result = {
            error: 'Unknown prices subcommand. Use: list, get, create, update',
          }
      }
      break
    ...
    case 'customers':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown customers subcommand. Use: list, get, create, update',
          }
      }
      break
    ...
    case 'subscriptions':
      switch (sub) {
        ...
        case 'update': {
          ...
          const body = {}
          if (args['proration-billing-mode'])
            body.proration_billing_mode = args['proration-billing-mode']
          if (args['scheduled-change']) {
            try {
              body.scheduled_change = JSON.parse(args['scheduled-change'])
            } catch {
              result = { error: 'Invalid JSON in --scheduled-change' }
              break
            }
          }
        ...
        case 'cancel': {
          ...
          const body = {
            effective_from: args['effective-from'] || 'next_billing_period',
          }
        ...
        case 'resume': {
          ...
          const body = {
            effective_from: args['effective-from'] || 'immediately',
          }
        ...
        default:
          result = {
            error:
              'Unknown subscriptions subcommand. Use: list, get, update, cancel, pause, resume',
          }
      }
      break
    ...
    case 'transactions':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!items) {
            result = {
              error: '--items required (JSON array of {price_id, quantity})',
            }
            break
          }
          let parsedItems
          try {
            ...
            result = { error: 'Invalid JSON in --items' }
            break
          }
        ...
        default:
          result = {
            error: 'Unknown transactions subcommand. Use: list, get, create',
          }
      }
      break
    ...
    case 'discounts':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!amount) {
            result = { error: '--amount required' }
            break
          }
          if (!type) {
            result = {
              error: '--type required (flat, flat_per_seat, percentage)',
            }
            break
          }
          const body = {
          ...
          if (args['max-uses'])
            body.maximum_recurring_intervals = Number(args['max-uses'])
        ...
        default:
          result = {
            error: 'Unknown discounts subcommand. Use: list, get, create',
          }
      }
      break
    ...
    case 'adjustments':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!transactionId) {
            result = { error: '--transaction-id required' }
            break
          }
          if (!action) {
            result = { error: '--action required (refund, credit, chargeback)' }
            break
          }
          if (!reason) {
          ...
          if (!items) {
            result = {
              error: '--items required (JSON array of {item_id, type, amount})',
            }
            break
          }
          let parsedItems
        ...
        default:
          result = {
            error: 'Unknown adjustments subcommand. Use: list, create',
          }
      }
      break
    ...
    case 'notifications':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown notifications subcommand. Use: list, get, replay',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          products:
            'products [list | get --id <id> | create --name <n> --tax-category <cat> | update --id <id>]',
          prices:
            'prices [list | get --id <id> | create --product-id <id> --amount <amt> [--currency USD] [--interval month --frequency 1] | update --id <id>]',
          customers:
            'customers [list | get --id <id> | create --email <email> [--name <name>] | update --id <id>]',
          subscriptions:
            'subscriptions [list | get --id <id> | update --id <id> | cancel --id <id> [--effective-from next_billing_period] | pause --id <id> | resume --id <id>]',
          transactions:
            'transactions [list | get --id <id> | create --items <json>]',
          discounts:
            'discounts [list | get --id <id> | create --amount <amt> --type <type> [--code <code>]]',
          adjustments:
            'adjustments [list | create --transaction-id <id> --action <action> --reason <reason> --items <json>]',
          events: 'events [list | types]',
          notifications:
            'notifications [list | get --id <id> | replay --id <id>]',

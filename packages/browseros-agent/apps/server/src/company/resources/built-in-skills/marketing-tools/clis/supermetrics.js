...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'SUPERMETRICS_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
...
async function main() {
  ...
  switch (cmd) {
    case 'query': {
      ...
      if (!dsId) {
        result = { error: '--ds-id required (e.g., GA4, AW, FB, LI, GSC)' }
        break
      }
      if (!dsAccounts) {
        result = { error: '--ds-accounts required' }
        break
      }
      if (!dateRange) {
        result = {
          error:
            '--date-range required (e.g., last_28_days, last_month, this_month, custom)',
        }
        break
      }
      if (!fields) {
        result = { error: '--fields required (comma-separated field names)' }
        break
      }
      const body = {
        ds_id: dsId,
    ...
    case 'accounts':
      switch (sub) {
        case 'list': {
          const dsId = args['ds-id']
          if (!dsId) {
            result = { error: '--ds-id required (e.g., GA4, AW, FB)' }
            break
          }
          const params = new URLSearchParams({ ds_id: dsId })
          result = await api(
            'GET',
            `/datasources/accounts?${params.toString()}`,
          )
          break
        }
        default:
    ...
    default:
      result = {
        ...
        usage: {
          query:
            'query --ds-id <data-source> --ds-accounts <account-id> --date-range <range> --fields <f1,f2> [--filter <filter>] [--max-rows <n>] [--start-date <date>] [--end-date <date>]',
          sources: {
            list: 'sources list',
          },
          accounts: {

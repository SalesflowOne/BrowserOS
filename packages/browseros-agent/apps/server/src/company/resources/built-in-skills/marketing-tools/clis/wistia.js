...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'WISTIA_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'projects':
      switch (sub) {
        case 'list':
          result = await api(
            'GET',
            `/projects.json?page=${page}&per_page=${perPage}`,
          )
          break
        case 'get': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (project hashed ID)' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown projects subcommand. Use: list, get, create, update, delete',
          }
      }
      break
    ...
    case 'medias':
      switch (sub) {
        ...
        case 'get': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (media hashed ID)' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown medias subcommand. Use: list, get, update, delete, copy, stats',
          }
      }
      break
    ...
    case 'stats':
      switch (sub) {
        ...
        case 'visitor': {
          const key = args.key
          if (!key) {
            result = { error: '--key required (visitor key)' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown stats subcommand. Use: account, project, media, media-by-date, engagement, visitors, visitor, events',
          }
      }
      break
    ...
    case 'captions':
      switch (sub) {
        ...
        case 'create': {
          ...
          if (!language) {
            result = { error: '--language required (e.g. eng)' }
            break
          }
          const body = { language }
          if (args['srt-file']) {
    ...
    default:
      result = {
        ...
        usage: {
          projects:
            'projects [list | get --id <id> | create --name <name> | update --id <id> --name <name> | delete --id <id>]',
          medias:
            'medias [list [--project <id>] | get --id <id> | update --id <id> --name <name> | delete --id <id> | copy --id <id> [--target-project <id>] | stats --id <id>]',
          stats:
            'stats [account | project --id <id> | media --id <id> | media-by-date --id <id> [--start <date> --end <date>] | engagement --id <id> | visitors | visitor --key <key> | events [--media-id <id>]]',
          account: 'account',
          captions:
            'captions [list --id <media-id> | create --id <media-id> --language <lang>]',

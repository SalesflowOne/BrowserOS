...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'SENDGRID_API_KEY environment variable required' }),
  )
  process.exit(1)
}
    ...
    case 'send': {
      ...
      if (args['template-id']) {
        ...
        if (args['template-data']) {
          try {
            body.personalizations[0].dynamic_template_data = JSON.parse(
              args['template-data'],
            )
          } catch (e) {
            result = { error: 'Invalid JSON for --template-data: ' + e.message }
            break
          }
        }
      } else {
      ...
      if (args.cc)
        body.personalizations[0].cc = args.cc
          .split(',')
          .map((e) => ({ email: e.trim() }))
      if (args.bcc)
        body.personalizations[0].bcc = args.bcc
          .split(',')
          .map((e) => ({ email: e.trim() }))
    ...
    case 'contacts':
      switch (sub) {
        ...
        case 'add': {
          ...
          if (args['first-name'])
            body.contacts[0].first_name = args['first-name']
        ...
        default:
          result = {
            error: 'Unknown contacts subcommand. Use: list, add, search',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          ...
          contacts:
            'contacts [list|add|search] [--email <email>] [--first-name <name>] [--last-name <name>] [--list-ids <ids>] [--query <sgql>]',
          campaigns: 'campaigns [list|get] [id] [--limit <n>]',
          stats:
            'stats get [--start-date <YYYY-MM-DD>] [--end-date <YYYY-MM-DD>]',
          bounces:
            'bounces list [--start-time <ts>] [--end-time <ts>] [--limit <n>]',
          'spam-reports':
            'spam-reports list [--start-time <ts>] [--end-time <ts>] [--limit <n>]',

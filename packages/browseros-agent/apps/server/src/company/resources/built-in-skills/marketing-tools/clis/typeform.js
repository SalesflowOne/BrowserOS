...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'TYPEFORM_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
      ...
      switch (sub) {
        case 'list': {
          ...
          if (args['workspace-id'])
            params.set('workspace_id', args['workspace-id'])
        ...
        case 'get': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (form ID)' }
            break
          }
        ...
        case 'create': {
          ...
          if (args['workspace-id']) {
            body.workspace = {
              href: `${BASE_URL}/workspaces/${args['workspace-id']}`,
            }
          }
        ...
        default:
          result = {
            error:
              'Unknown forms subcommand. Use: list, get, create, update, delete',
          }
      }
      break
    ...
    case 'responses':
      switch (sub) {
        case 'list': {
          ...
          if (args['response-type'])
            params.set('response_type', args['response-type'])
        ...
        case 'delete': {
          ...
          if (!responseIds) {
            result = { error: '--response-ids required (comma-separated)' }
            break
          }
          result = await api(
            'DELETE',
            `/forms/${id}/responses?included_response_ids=${encodeURIComponent(responseIds)}`,
          )
          break
        }
        default:
    ...
    case 'webhooks':
      switch (sub) {
        ...
        case 'get': {
          ...
          const tag = args.tag
          if (!id || !tag) {
            result = { error: '--id (form ID) and --tag required' }
            break
          }
        ...
        case 'create': {
          ...
          const url = args.url
          if (!id || !tag || !url) {
            result = { error: '--id (form ID), --tag, and --url required' }
            break
          }
    ...
    case 'themes':
      switch (sub) {
        ...
        case 'get': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (theme ID)' }
            break
          }
        ...
        default:
          result = {
            error: 'Unknown themes subcommand. Use: list, get, create, delete',
          }
      }
      break
    ...
    case 'images':
      switch (sub) {
        ...
        case 'get': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (image ID)' }
            break
          }
    ...
    case 'workspaces':
      switch (sub) {
        ...
        case 'get': {
          const id = args.id
          if (!id) {
            result = { error: '--id required (workspace ID)' }
            break
          }
    ...
    default:
      result = {
        ...
        usage: {
          forms:
            'forms [list | get --id <id> | create --title <title> | update --id <id> --title <title> | delete --id <id>]',
          responses:
            'responses [list --id <form_id> | delete --id <form_id> --response-ids <id1,id2>]',
          webhooks:
            'webhooks [list --id <form_id> | get --id <form_id> --tag <tag> | create --id <form_id> --tag <tag> --url <url> | delete --id <form_id> --tag <tag>]',
          themes:
            'themes [list | get --id <id> | create --name <name> | delete --id <id>]',
          images: 'images [list | get --id <id>]',
          workspaces: 'workspaces [list | get --id <id>]',
          options:
            '--page-size <n> --page <n> --since <iso> --until <iso> --query <text>',
        },
      }
  }

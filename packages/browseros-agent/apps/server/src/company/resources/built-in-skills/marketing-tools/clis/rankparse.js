...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'RANKPARSE_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
...
async function main() {
  ...
  switch (cmd) {
    ...
    case 'crawl-history': {
      ...
      result = await api(
        'GET',
        `/${cmd}?domain=${encodeURIComponent(args.domain)}`,
      )
      break
    }
    ...
    case 'domain-overlap': {
      if (!args.domains) {
        result = { error: '--domains required (comma-separated, 2-5 domains)' }
        break
      }
    ...
    case 'link-intersect': {
      if (!args['domain-a'] || !args['domain-b']) {
        result = { error: '--domain-a and --domain-b required' }
        break
      }
      const params = new URLSearchParams({
        domain_a: args['domain-a'],
        domain_b: args['domain-b'],
      })
      if (args.limit) params.set('limit', args.limit)
    ...
    case 'competitor-gap': {
      ...
      if (!args.vs) {
        result = { error: '--vs required (competitor domain)' }
        break
      }
    ...
    default:
      result = {
        ...
        usage: {
          ...
          backlinks:
            'backlinks --domain <domain> [--limit <n>] [--sort importance|recent] [--from-domain <d>] [--link-type <t>] [--score]',
          'referring-domains':
            'referring-domains --domain <domain> [--limit <n>]',
          ...
          'domain-overlap':
            'domain-overlap --domains <d1,d2,...> [--limit <n>]',
          'link-intersect':
            'link-intersect --domain-a <d> --domain-b <d> [--limit <n>]',
          'competitor-gap':
            'competitor-gap --domain <d> --vs <competitor> [--limit <n>]',
          ...
          'page-performance':
            'page-performance --url <url> [--strategy mobile|desktop]',

...
async function main() {
  ...
  const locationCode = args['location-code']
    ? Number(args['location-code'])
    : 2840
  ...
  switch (cmd) {
    case 'serp':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown serp subcommand. Use: google, locations, languages',
          }
      }
      break
    ...
    case 'keywords':
      switch (sub) {
        case 'volume': {
          ...
          if (!keywords) {
            result = { error: '--keywords required (comma-separated)' }
            break
          }
          result = await api(
            'POST',
        ...
        case 'for-site': {
          ...
          if (!target) {
            result = { error: '--target required (domain)' }
            break
          }
          result = await api(
            'POST',
        ...
        default:
          result = {
            error:
              'Unknown keywords subcommand. Use: volume, for-site, for-keywords, trends',
          }
      }
      break
    ...
    case 'backlinks':
      switch (sub) {
        ...
        default:
          result = {
            error:
              'Unknown backlinks subcommand. Use: summary, list, refdomains, anchors, index',
          }
      }
      break
      ...
      switch (sub) {
        ...
        case 'domain-intersection': {
          ...
          if (!targets || targets.length < 2) {
            result = {
              error: '--targets required (comma-separated, at least 2 domains)',
            }
            break
          }
          const payload = {
        ...
        default:
          result = {
            error:
              'Unknown labs subcommand. Use: competitors, ranked-keywords, domain-intersection',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          ...
          keywords:
            'keywords [volume --keywords <kw1,kw2> | for-site --target <domain> | for-keywords --keywords <kw1,kw2> | trends --keywords <kw1,kw2>]',
          backlinks:
            'backlinks [summary --target <domain> | list --target <domain> | refdomains --target <domain> | anchors --target <domain> | index]',

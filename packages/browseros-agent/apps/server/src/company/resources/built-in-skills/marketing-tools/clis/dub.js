...
if (!API_KEY) {
  console.error(
    JSON.stringify({ error: 'DUB_API_KEY environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'links':
      switch (sub) {
        ...
        case 'update': {
          if (!args.id) {
            result = { error: '--id required (link ID)' }
            break
          }
          const body = {}
          if (args.url) body.url = args.url
        ...
        case 'bulk-create': {
          ...
          try {
            ...
            result = { error: 'Invalid JSON in --links' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown links subcommand. Use: create, list, get, update, delete, bulk-create',
          }
      }
      break
    ...
    case 'analytics':
      switch (sub) {
        ...
        default:
          result = {
            error: 'Unknown analytics subcommand. Use: get, country, device',
          }
      }
      break
    ...
    default:
      result = {
        ...
        usage: {
          links:
            'links [create|list|get|update|delete|bulk-create] [--url <url>] [--domain <domain>] [--key <key>] [--tags <tags>] [--id <id>] [--page <page>] [--links <json>]',
          analytics:
            'analytics [get|country|device] [--domain <domain>] [--key <key>] [--interval <interval>]',
        },
      }
  }

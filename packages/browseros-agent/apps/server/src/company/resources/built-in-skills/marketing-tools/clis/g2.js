...
if (!API_TOKEN) {
  console.error(
    JSON.stringify({ error: 'G2_API_TOKEN environment variable required' }),
  )
  process.exit(1)
}
...
async function main() {
  ...
  switch (cmd) {
    case 'reviews':
      switch (sub) {
        case 'list': {
          ...
          if (args.state)
            qs += `&filter[state]=${encodeURIComponent(args.state)}`
    ...
    case 'reports':
      switch (sub) {
        case 'list': {
          const qs = `?page[size]=${perPage}&page[number]=${page}`
    ...
    case 'competitors':
      switch (sub) {
        case 'list': {
          ...
          const qs = `?page[size]=${perPage}&page[number]=${page}&filter[product_id]=${productId}`
    ...
    case 'tracking':
      switch (sub) {
        case 'visitors': {
          let qs = `?page[size]=${perPage}&page[number]=${page}`
          if (args.start)
            qs += `&filter[start_date]=${encodeURIComponent(args.start)}`
          if (args.end)
            qs += `&filter[end_date]=${encodeURIComponent(args.end)}`

...
if (!TOKEN) {
  console.error(
    JSON.stringify({
      error: 'TIKTOK_ACCESS_TOKEN environment variable required',
    }),
  )
  process.exit(1)
}
    ...
    case 'advertiser':
      switch (sub) {
        case 'info': {
          ...
          if (!advId) {
            result = {
              error: 'TIKTOK_ADVERTISER_ID env or --advertiser-id required',
            }
            break
          }
          const advParams = new URLSearchParams({
            advertiser_ids: JSON.stringify([advId]),
          })
    ...
    case 'campaigns':
      switch (sub) {
        ...
        case 'update-status': {
          ...
          if (!args.ids || !args.status) {
            result = { error: '--ids and --status required' }
            break
          }
        ...
        default:
          result = {
            error:
              'Unknown campaigns subcommand. Use: list, create, update-status',
          }
      }
      break
    ...
    case 'adgroups':
      switch (sub) {
        case 'list': {
          ...
          if (args['campaign-id'])
            agParams.set('campaign_ids', JSON.stringify([args['campaign-id']]))
      ...
      switch (sub) {
        case 'get': {
          ...
          if (!args['start-date'] || !args['end-date']) {
            result = {
              error: '--start-date and --end-date required (YYYY-MM-DD)',
            }
            break
          }
          const body = {
    ...
    default:
      result = {
        ...
        usage: {
          ...
          campaigns:
            'campaigns [list|create|update-status] [--name <name>] [--objective <obj>] [--budget-mode BUDGET_MODE_DAY] [--budget <amount>] [--ids <id1,id2>] [--status ENABLE|DISABLE]',
          adgroups: 'adgroups [list] [--campaign-id <id>]',
          reports:
            'reports [get] --start-date YYYY-MM-DD --end-date YYYY-MM-DD [--dimensions campaign_id] [--metrics spend,impressions,clicks,conversion] [--data-level AUCTION_CAMPAIGN]',

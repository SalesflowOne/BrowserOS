...
if (!CLIENT_ID || !CLIENT_SECRET) {
  console.error(
    JSON.stringify({
      error:
        'HOTJAR_CLIENT_ID and HOTJAR_CLIENT_SECRET environment variables required',
    }),
  )
  process.exit(1)
}
    ...
    case 'surveys':
      ...
      switch (sub) {
        ...
        case 'responses': {
          ...
          if (!surveyId) {
            result = { error: '--survey-id required' }
            break
          }
          ...
          result = await api(
            'GET',
            `/sites/${siteId}/surveys/${surveyId}/responses?${params.toString()}`,
          )
          break
        }
        default:
    ...
    case 'recordings':
      ...
      switch (sub) {
        case 'list': {
          ...
          result = await api(
            'GET',
            `/sites/${siteId}/recordings?${params.toString()}`,
          )
          break
        }
        default:
    ...
    default:
      result = {
        ...
        usage: {
          sites: 'sites list',
          surveys:
            'surveys list --site-id <id> | surveys responses --site-id <id> --survey-id <id> [--limit <n>] [--cursor <cursor>]',
          heatmaps: 'heatmaps list --site-id <id>',
          recordings:
            'recordings list --site-id <id> [--limit <n>] [--cursor <cursor>] [--date-from <date>] [--date-to <date>]',

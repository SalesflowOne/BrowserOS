...
if (!API_KEY) {
  console.error(
    JSON.stringify({
      error: 'AMPLITUDE_API_KEY environment variable required',
    }),
  )
  process.exit(1)
}
...
async function queryApi(method, path, params) {
  if (!SECRET_KEY) {
    return {
      error: 'AMPLITUDE_SECRET_KEY required for query/export operations',
    }
  }
  ...
  switch (cmd) {
    case 'track':
      switch (sub) {
        case 'event': {
          ...
          if (!args['event-type']) {
            result = { error: '--event-type required' }
            break
          }
          const event = {
    ...
    case 'export':
      switch (sub) {
        case 'events': {
          if (!args.start) {
            result = { error: '--start required (e.g. 20240101T00)' }
            break
          }
          if (!args.end) {
            result = { error: '--end required (e.g. 20240131T23)' }
            break
          }
          const params = new URLSearchParams()
    ...
    case 'retention':
      switch (sub) {
        case 'get': {
          if (!args.start) {
            result = { error: '--start required (e.g. 20240101)' }
            break
          }
          if (!args.end) {
            result = { error: '--end required (e.g. 20240131)' }
            break
          }
          const params = new URLSearchParams()
          params.set('start', args.start)
    ...
    default:
      result = {
        ...
        usage: {
          track:
            'track [event --user-id <id> --event-type <type> [--properties <json>] | batch --events <json>]',
          users: 'users activity --user-id <id>',
          export: 'export events --start <YYYYMMDDThh> --end <YYYYMMDDThh>',
          retention:
            'retention get --start <YYYYMMDD> --end <YYYYMMDD> [--event <type>]',
        },
      }
  }

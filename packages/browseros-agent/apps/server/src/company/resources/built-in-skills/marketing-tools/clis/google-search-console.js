...
if (!ACCESS_TOKEN) {
  console.error(
    JSON.stringify({ error: 'GSC_ACCESS_TOKEN environment variable required' }),
  )
  process.exit(1)
}
  ...
  switch (cmd) {
    case 'search': {
      ...
      switch (sub) {
        case 'query':
          ...
          result = await api(
            'POST',
            `/webmasters/v3/sites/${encodedSiteUrl}/searchAnalytics/query`,
            body,
          )
          break
        case 'pages':
        ...
        default:
          result = {
            error: 'Unknown search subcommand. Use: query, pages, countries',
          }
      }
      break
    }
    ...
    case 'inspect': {
      ...
      switch (sub) {
        case 'url':
          if (!args.url) {
            result = { error: '--url required (URL to inspect)' }
            break
          }
    ...
    case 'sitemaps': {
      ...
      switch (sub) {
        case 'list':
          result = await api(
            'GET',
            `/webmasters/v3/sites/${encodedSiteUrl}/sitemaps`,
          )
          break
        case 'submit': {
          if (!args['sitemap-url']) {
            result = { error: '--sitemap-url required' }
            break
          }
          const sitemapUrl = encodeURIComponent(args['sitemap-url'])
          result = await api(
            'PUT',
            `/webmasters/v3/sites/${encodedSiteUrl}/sitemaps/${sitemapUrl}`,
          )
    ...
    default:
      result = {
        ...
        usage: {
          'search query':
            'search query --site-url <url> [--start-date <date>] [--end-date <date>] [--limit <n>]',
          'search pages':
            'search pages --site-url <url> [--start-date <date>] [--end-date <date>] [--limit <n>]',
          'search countries':
            'search countries --site-url <url> [--start-date <date>] [--end-date <date>] [--limit <n>]',
          'inspect url': 'inspect url --site-url <url> --url <page-url>',
          'sitemaps list': 'sitemaps list --site-url <url>',
          'sitemaps submit':
            'sitemaps submit --site-url <url> --sitemap-url <sitemap-url>',
        },
      }
  }

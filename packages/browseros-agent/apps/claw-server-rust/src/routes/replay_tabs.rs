use crate::{services::tab_activity::TabActivityService, sessions::Sessions, tabs::TabGroupColor};
use serde::Serialize;
...
pub async fn list_replay_tabs(
    ...
    for record in records {
        ...
        let convo_id = session.convo_id().clone();
        tabs.push(ReplayTab {
            session_id: session.id().as_str().to_string(),
            tab_page_id: record.page_id,
            url: record.url,
            title: record.title,
            group_color: sessions.ownership().tab_group_color(&convo_id).await,
        });
    }
